...
function OpenVigneActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vigne_actions', {
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('metaverse_society:openBossMenu', 'vigne', function(data, menu)
					menu.close()
				end)
			end
		end, function(data, menu)
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('metaverse_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
					...
					TriggerServerEvent('metaverse_society:removeVehicleFromGarage', 'vigne', vehicleProps)
				end, function(data, menu)
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('metaverse_factures:sendBill', GetPlayerServerId(closestPlayer), 'society_vigne', 'Vigneron', amount)
								end)
							end
						end
					end, function(data, menu)
			...
			if IsControlPressed(0,  38) and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' and (GetGameTimer() - GUI.Time) > 300 then
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('metaverse_society:putVehicleInGarage', 'vigne', vehicleProps)
					end
