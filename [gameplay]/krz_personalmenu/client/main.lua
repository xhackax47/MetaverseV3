...
RegisterNetEvent('metaverse_addonaccount:setMoney')
AddEventHandler('metaverse_addonaccount:setMoney', function(society, money)
...
function RefreshMoney()
	if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.grade_name == 'boss' then
		ESX.TriggerServerCallback('metaverse_society:getSocietyMoney', function(money)
			societymoney = ESX.Math.GroupDigits(money)
	...
	RageUI.DrawContent({header = true, instructionalButton = true}, function()
		for i = 1, #PersonalMenu.BillData, 1 do
			RageUI.Button(PersonalMenu.BillData[i].label, nil, {RightLabel = '$' .. ESX.Math.GroupDigits(PersonalMenu.BillData[i].amount)}, true, function(Hovered, Active, Selected)
				if (Selected) then
					ESX.TriggerServerCallback('metaverse_factures:payBill', function()
