...
for i,entry in ipairs(registerStrings) do
    AddTextEntry('plants_'..entry, _U(entry))
end

function interactHelp(stage,action)
    BeginTextCommandDisplayHelp('plants_status_active')
...
function passiveHelp(stage,status)
    BeginTextCommandDisplayHelp('plants_status_passive')
    AddTextComponentInteger(stage)
    AddTextComponentInteger(#Growth)
...
function serverlog(...)
    TriggerServerEvent('metaverse_plants:log',...)
end

RegisterNetEvent('metaverse_plants:make_toast')
AddEventHandler ('metaverse_plants:make_toast', function(subject,message)
...
RegisterNetEvent('metaverse_plants:do')
AddEventHandler ('metaverse_plants:do', function(scenarioName, location)
...
RegisterNetEvent('metaverse_plants:attempt_plant')
AddEventHandler ('metaverse_plants:attempt_plant', function()
    local plantable, message, location, _, soil = getPlantingLocation()
    if plantable then
        TriggerServerEvent('metaverse_plants:success_plant', location, soil)
                ...
                if closestDistance <= Config.Distance.Interact then
                    ...
                    if now >= lastAction + Config.ActionTime then
                        if IsDisabledControlJustPressed(0, 44) then
                            ...
                            TriggerServerEvent('metaverse_plants:remove', closestPlant.id, myLocation)
                        else
                            if stage.interact then
                                interactHelp(closestPlant.stage, _U(stage.label))
                                if IsControlJustPressed(0, 38) then
                                    lastAction = now
                                    TriggerServerEvent('metaverse_plants:frob', closestPlant.id, myLocation)
                                end
                            else
...
RegisterNetEvent('metaverse_plants:toggle_debug')
AddEventHandler ('metaverse_plants:toggle_debug', function()
...
RegisterNetEvent('metaverse_plants:pyromaniac')
AddEventHandler ('metaverse_plants:pyromaniac',function(location)
            ...
            Citizen.CreateThread(function()
                ...
                if not HasNamedPtfxAssetLoaded(Config.Burn.Effect) then
                    ...
                    if not HasNamedPtfxAssetLoaded(Config.Burn.Collection) then
                        print("plants failed to load particle effects asset "..Config.Burn.Collection)
                    end
                end
...
RegisterNetEvent('metaverse_plants:groundmat')
AddEventHandler ('metaverse_plants:groundmat', function()
    local plantable, message, where, normal, material = getPlantingLocation(true)
...
RegisterNetEvent('metaverse_plants:test_forest')
AddEventHandler ('metaverse_plants:test_forest',function(count, randomStage)
    local origin = GetEntityCoords(PlayerPedId())
    
    TriggerEvent("chat:addMessage", {args={'plants','Target forest size: '..count}})
    local column = math.ceil(math.sqrt(count))
    TriggerEvent("chat:addMessage", {args={'plants','Column size: '..column}})
    ...
    TriggerEvent("chat:addMessage", {args={'plants', 'Actual viable locations: '..#forest}})
    TriggerServerEvent('metaverse_plants:test_forest', forest)
end)
