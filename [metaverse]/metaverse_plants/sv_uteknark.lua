...
function makeToast(target, subject, message)
    TriggerClientEvent('metaverse_plants:make_toast', target, subject, message)
end
function inChat(target, message)
    if target == 0 then
        log(message)
    else
        TriggerClientEvent('chat:addMessage',target,{args={'plants', message}})
    end
end
...
function doScenario(who, what, where)
    ...
    TriggerClientEvent('metaverse_plants:do', who, what, where)
end

RegisterNetEvent('metaverse_plants:success_plant')
AddEventHandler ('metaverse_plants:success_plant', function(location, soil)
    local src = source
...
RegisterNetEvent('metaverse_plants:log')
AddEventHandler ('metaverse_plants:log',function(...)
    local src = source
    log(src,GetPlayerName(src),...)
end)

RegisterNetEvent('metaverse_plants:test_forest')
AddEventHandler ('metaverse_plants:test_forest',function(forest)
    local src = source


    if IsPlayerAceAllowed(src, 'command.plants') then
                ...
                ESX.RegisterUsableItem(Config.Items.Seed, function(source)
                    ...
                    if now > last + (Config.ActionTime/1000) then
                        if HasItem(source, Config.Items.Seed) then
                            TriggerClientEvent('metaverse_plants:attempt_plant', source)
        ...
        if state == "started" then
            ...
            cropstate:load(function(plantCount)
                if plantCount == 1 then
                    log('plants loaded a single plant!')
                else
                    log('plants loaded',plantCount,'plants')
                end
            end)
            databaseReady = true
        end
...
local commands = {
    debug = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('metaverse_plants:toggle_debug', source)
        end
    end,
    ...
    forest = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('metaverse_plants:test_forest', source, count, randomStage)
    ...
    groundmat = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('metaverse_plants:groundmat', source)
        end
    end,
    pyro = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('metaverse_plants:pyromaniac', source)
        end
    end,
}

RegisterCommand('plants', function(source, args, raw)
    if #args > 0 then
