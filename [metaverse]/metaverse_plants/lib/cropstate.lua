...
local cropstateMethods = {
    plant = function(instance, location, soil, stage)
        if onServer then
            stage = stage or 1
            MySQL.Async.insert("INSERT INTO `plants` (`x`, `y`, `z`, `soil`, `stage`) VALUES (@x, @y, @z, @soil, @stage);",
            {
                ['@x'] = location.x,
                ['@y'] = location.y,
                ['@z'] = location.z,
            ...
            function(id)
                ...
                TriggerClientEvent('metaverse_plants:planted',-1, id, location, stage)
    ...
    load = function(instance, callback)
        if onServer then
            verbose('Loading...')
            MySQL.Async.fetchAll("SELECT `id`, `stage`, UNIX_TIMESTAMP(`time`) AS `time`, `x`, `y`, `z`, `soil` FROM `plants`;", 
            {},
            function(rows)
    ...
    import = function(instance, id, location, stage, time, soil)
        ...
        if not success then
            Citizen.Trace(string.format("plants failed to import plant with ID %i into octree\n", id))
        end
    ...
    update = function(instance, id, stage)
        ...
        if onServer then
            ...
            MySQL.Async.execute("UPDATE `plants` SET `stage` = @stage WHERE `id` = @id LIMIT 1;",
            {
                ['@id'] = id,
                ['@stage'] = stage,
            }, function(_)
                TriggerClientEvent('metaverse_plants:update', -1, id, stage)
    ...
    remove = function(instance, id)
        ...
        if onServer then
            MySQL.Async.execute("DELETE FROM `plants` WHERE `id` = @id LIMIT 1;",
            { ['@id'] = id },
            function()
                TriggerClientEvent('metaverse_plants:removePlant', -1, id)
                TriggerClientEvent('metaverse_plants:pyromaniac', -1, location)
    ...
    bulkData = function(instance, target)
        if onServer then
            ...
            TriggerClientEvent('metaverse_plants:bulk_data', target, forest)
        else
            TriggerServerEvent('metaverse_plants:request_data')
        end
    end,
}
...
if onServer then
    RegisterNetEvent('metaverse_plants:request_data')
    AddEventHandler ('metaverse_plants:request_data', function()
        cropstate:bulkData(source)
    end)
    
    RegisterNetEvent('metaverse_plants:remove')
    AddEventHandler ('metaverse_plants:remove', function(plantID, nearLocation)
        local src = source
        ...
        if plant then
            ...
            TriggerClientEvent('metaverse_plants:remove', src, plantID)
        end
    end)

    RegisterNetEvent('metaverse_plants:frob')
    AddEventHandler ('metaverse_plants:frob', function(plantID, nearLocation)
        local src = source
        local plant = cropstate.index[plantID]
        if plant then
            local plantLocation = plant.bounds.location
    ...
    RegisterNetEvent('metaverse_plants:bulk_data')
    AddEventHandler ('metaverse_plants:bulk_data', function(forest)
    ...
    RegisterNetEvent('metaverse_plants:planted')
    AddEventHandler ('metaverse_plants:planted', function(id, location, stage)
        cropstate:import(id, location, stage)
    end)
    
    RegisterNetEvent('metaverse_plants:update')
    AddEventHandler ('metaverse_plants:update', function(plantID, stage)
        cropstate:update(plantID, stage)
    end)

    RegisterNetEvent('metaverse_plants:removePlant')
    AddEventHandler ('metaverse_plants:removePlant', function(plantID)
        cropstate:remove(plantID)
    end)
end
