Config = {}
Config.DrawDistance = 10.0
Config.MarkerSize = {x = 1.1, y = 0.7, z = 1.1}
Config.MarkerType =  29
Config.MarkerColor = {r = 50, g = 200, b = 50, a = 200}
Config.Locale = GetConvar('esx:locale', 'en')

Config.Zones = {

	TwentyFourSeven = {
		Items = {
			{
				name = "bread",
				label = TranslateCap('bread'),
				price = 25
			},
			{
				name = "water",
				label = TranslateCap('water'),
				price = 25
			},
			{
				name = "beer",
				label = TranslateCap('beer'),
				price = 50
			},
			{
				name = "croquettes",
				label = TranslateCap('croquettes'),
				price = 100
			},
			{
				name = "vape",
				label = TranslateCap('vape'),
				price = 250
			},
			{
				name = "phone",
				label = TranslateCap('phone'),
				price = 500
			},
			{
				name = "radio",
				label = TranslateCap('radio'),
				price = 500
			},
			{
				name = "sim_card",
				label = TranslateCap('sim_card'),
				price = 1000
			},
			{
				name = "bag",
				label = TranslateCap('bag'),
				price = 1000
			},
			{
				name = "autoradio",
				label = TranslateCap('autoradio'),
				price = 20000
			}
		},
		Pos = {
			vector3(373.8, 325.8, 103.5),
			vector3(2557.4, 382.2, 108.6),
			vector3(-3038.9, 585.9, 7.9),
			vector3(-3241.9, 1001.4, 12.8),
			vector3(547.4, 2671.7, 42.1),
			vector3(1961.4, 3740.6, 32.3),
			vector3(2678.9, 3280.6, 55.2),
			vector3(1729.2, 6414.1, 35.0)
	},
	Size  = 0.8,
	Type  = 59,
	Color = 25,
	ShowBlip = true,
  ShowMarker = false
},

	RobsLiquor = {
		Items = {
			{
				name = "bread",
				label = TranslateCap('bread'),
				price = 25
			},
			{
				name = "water",
				label = TranslateCap('water'),
				price = 25
			},
			{
				name = "beer",
				label = TranslateCap('beer'),
				price = 50
			},
			{
				name = "croquettes",
				label = TranslateCap('croquettes'),
				price = 100
			},
			{
				name = "vape",
				label = TranslateCap('vape'),
				price = 250
			},
			{
				name = "phone",
				label = TranslateCap('phone'),
				price = 500
			},
			{
				name = "radio",
				label = TranslateCap('radio'),
				price = 500
			},
			{
				name = "sim_card",
				label = TranslateCap('sim_card'),
				price = 1000
			},
			{
				name = "bag",
				label = TranslateCap('bag'),
				price = 1000
			},
			{
				name = "autoradio",
				label = TranslateCap('autoradio'),
				price = 20000
			}
		},
		Pos = {
			vector3(1135.8, -982.2, 46.4),
			vector3(-1222.9, -906.9, 12.3),
			vector3(-1487.5, -379.1, 40.1),
			vector3(-2968.2, 390.9, 15.0),
			vector3(1166.0, 2708.9, 38.1),
			--vector3(1392.5, 3604.6,  34.9), -- MC Bar AOC
			--vector3(127.8,  -1284.7, 29.2), -- Unicorn StripClub
			vector3(-1393.4, -606.6, 30.3), --Bahamamas
			--vector3(-559.9, 287.0, 82.1) --Tequila la
	},
	Size  = 0.8,
	Type  = 59,
	Color = 25,
	ShowBlip = true,
  ShowMarker = false
},

	LTDgasoline = {
		Items = {
			{
				name = "bread",
				label = TranslateCap('bread'),
				price = 25
			},
			{
				name = "water",
				label = TranslateCap('water'),
				price = 25
			},
			{
				name = "beer",
				label = TranslateCap('beer'),
				price = 50
			},
			{
				name = "croquettes",
				label = TranslateCap('croquettes'),
				price = 100
			},
			{
				name = "vape",
				label = TranslateCap('vape'),
				price = 250
			},
			{
				name = "phone",
				label = TranslateCap('phone'),
				price = 500
			},
			{
				name = "radio",
				label = TranslateCap('radio'),
				price = 500
			},
			{
				name = "sim_card",
				label = TranslateCap('sim_card'),
				price = 1000
			},
			{
				name = "bag",
				label = TranslateCap('bag'),
				price = 1000
			},
			{
				name = "autoradio",
				label = TranslateCap('autoradio'),
				price = 20000
			}
		},
		Pos = {
			vector3(-48.5,  -1757.5, 29.4),
			vector3(1163.3, -323.8, 69.2),
			vector3(-707.5, -914.2, 19.2),
			vector3(-1820.5, 792.5, 138.1),
			vector3(1698.3, 4924.4, 42.0)
	},
	Size  = 0.8,
	Type  = 59,
	Color = 25,
	ShowBlip = true,
  	ShowMarker = false
}
}

