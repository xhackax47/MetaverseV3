...
function OpenShopMenu(zone)
	...
	ESX.OpenContext("right", elements, function(menu,element)
		...
		ESX.OpenContext("right", elements2, function(menu2,element2)
			...
			ESX.CloseContext()
			TriggerServerEvent('metaverse_shops:buyItem', element.item, amount, zone)
		end, function(menu)
...
AddEventHandler('metaverse_shops:hasEnteredMarker', function(zone)
	currentAction     = 'shop_menu'
	currentActionMsg  = TranslateCap('press_menu')
	currentActionData = {zone = zone}
end)

AddEventHandler('metaverse_shops:hasExitedMarker', function(zone)
	currentAction = nil
	ESX.CloseContext()
end)
...
CreateThread(function()
	while true do
		...
		if isInMarker and not hasAlreadyEnteredMarker then
			...
			TriggerEvent('metaverse_shops:hasEnteredMarker', currentZone)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			ESX.HideUI()
			TriggerEvent('metaverse_shops:hasExitedMarker', lastZone)
		end
	Wait(Sleep)
	end
end)
