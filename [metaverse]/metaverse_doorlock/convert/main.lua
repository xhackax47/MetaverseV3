...
MySQL.ready(function()
	...
	if #files > 0 then
		for i = 1, #files do
			...
			local file = LoadResourceFile('metaverse_doorlock', ('convert/%s.lua'):format(fileName))
			...
			if file then
				...
				if next(Config.DoorList) then
					local size = 0
					local query = 'INSERT INTO metaverse_doorlock (name, data) VALUES (?, ?)'
					local queries = {}
					...
					if MySQL.transaction.await(queries) then
						...
						SaveResourceFile('metaverse_doorlock', ('convert/%s.lua'):format(fileName), '', -1)
					end
				end
			end
		end
