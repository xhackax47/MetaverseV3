...
exports('editDoor', function(id, data)
	...
	if door then
		...
		MySQL.update('UPDATE metaverse_doorlock SET name = ?, data = ? WHERE id = ?', { door.name, encodeData(door), id })
		TriggerClientEvent('metaverse_doorlock:editDoorlock', -1, id, door)
	end
end)
...
local function createDoor(id, door, name)
	...
	if type(door.items?[1]) == 'string' then
		...
		door.items = items
		MySQL.update('UPDATE metaverse_doorlock SET data = ? WHERE id = ?', { encodeData(door), id })
	end
...
local function isAuthorised(playerId, door, lockpick)
	...
	if player then
		...
		if authorised ~= nil and door.passcode then
			authorised = door.passcode == lib.callback.await('metaverse_doorlock:inputPassCode', playerId)
		end
	end
...
MySQL.ready(function()
	...
	local success, result = pcall(MySQL.query.await, 'SELECT id, name, data FROM metaverse_doorlock') --[[@as any]]

	if not success then
		-- because some people can't run sql files
		success, result = pcall(MySQL.query, [[CREATE TABLE `metaverse_doorlock` (
		...
		print("Created table 'metaverse_doorlock' in MySQL database.")
	elseif result then
		for i = 1, #result do
			local door = result[i]
...
RegisterNetEvent('metaverse_doorlock:setState', function(id, state, lockpick)
	local door = doors[id]
	...
	if door then
		...
		if authorised then
			door.state = state
			TriggerClientEvent('metaverse_doorlock:setState', -1, id, state, source)
			...
			if door.autolock and state == 0 then
				SetTimeout(door.autolock * 1000, function()
					if door.state ~= 1 then
						...
						TriggerClientEvent('metaverse_doorlock:setState', -1, id, door.state)
						TriggerEvent('metaverse_doorlock:stateChanged', nil, door.id, door.state == 1)
					end
				end)
			end

			return TriggerEvent('metaverse_doorlock:stateChanged', source, door.id, state == 1, type(authorised) == 'string' and authorised)
		end
...
RegisterNetEvent('metaverse_doorlock:getDoors', function()
	local source = source
	while not isLoaded do Wait(100) end
	TriggerClientEvent('metaverse_doorlock:setDoors', source, doors, sounds)
end)

RegisterNetEvent('metaverse_doorlock:editDoorlock', function(id, data)
	if IsPlayerAceAllowed(source, 'command.doorlock') then
		...
		if id then
			if data then
				MySQL.update('UPDATE metaverse_doorlock SET name = ?, data = ? WHERE id = ?', { data.name, encodeData(data), id })
			else
				MySQL.update('DELETE FROM metaverse_doorlock WHERE id = ?', { id })
			end

			doors[id] = data
			TriggerClientEvent('metaverse_doorlock:editDoorlock', -1, id, data)
		else
			local insertId = MySQL.insert.await('INSERT INTO metaverse_doorlock (name, data) VALUES (?, ?)', { data.name, encodeData(data) })
			local door = createDoor(insertId, data, data.name)

			TriggerClientEvent('metaverse_doorlock:setState', -1, door.id, door.state, false, door)
		end
	end
end)

RegisterNetEvent('metaverse_doorlock:breakLockpick', function()
...
lib.addCommand('doorlock', {
	...
	TriggerClientEvent('metaverse_doorlock:triggeredCommand', source, args.closest)
end)
