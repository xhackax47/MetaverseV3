...
local function pickLock(entity)
	...
	if success then
		TriggerServerEvent('metaverse_doorlock:setState', door.id, door.state == 1 and 0 or 1, true)
	end

	if rand == 1 then
		TriggerServerEvent('metaverse_doorlock:breakLockpick')
...
RegisterNUICallback('createDoor', function(data, cb)
	...
	TriggerServerEvent('metaverse_doorlock:editDoorlock', data.id or false, data)
	table.wipe(tempData)
end)

RegisterNUICallback('deleteDoor', function(id, cb)
	cb(1)
	TriggerServerEvent('metaverse_doorlock:editDoorlock', id)
end)
...
RegisterNetEvent('metaverse_doorlock:triggeredCommand', function(closest)
