...
lib.locale()
TriggerServerEvent('metaverse_doorlock:getDoors')
...
RegisterNetEvent('metaverse_doorlock:setDoors', function(data, sounds)
	doors = data
...
RegisterNetEvent('metaverse_doorlock:setState', function(id, state, source, data)
...
lib.callback.register('metaverse_doorlock:inputPassCode', function()
...
local function useClosestDoor()
	...
	if gameTimer - lastTriggered > 500 then
		...
		TriggerServerEvent('metaverse_doorlock:setState', ClosestDoor.id, ClosestDoor.state == 1 and 0 or 1)
	end
end
