...
MySQL.ready(function()
	...
	Citizen.Await(p)
	ESX.Trace('[metaverse_licences] : ' .. #licenses .. ' Loaded.')
end)
...
AddEventHandler('metaverse_licences:addLicense', function(target, licenseType, cb)
	local xPlayer = ESX.GetPlayerFromId(target)
	if xPlayer then
		if isValidLicense(licenseType) then
			AddLicense(xPlayer.getIdentifier(), licenseType, cb)
		else
			print('[metaverse_licences] : missing license type in db ' .. licenseType .. ' or someone try to use lua executor id : ' .. target)
		end
	end
end)

RegisterNetEvent('metaverse_licences:removeLicense')
AddEventHandler('metaverse_licences:removeLicense', function(target, licenseType, cb)
...
AddEventHandler('metaverse_licences:getLicense', function(licenseType, cb)
	GetLicense(licenseType, cb)
end)

AddEventHandler('metaverse_licences:getLicenses', function(target, cb)
	local xPlayer = ESX.GetPlayerFromId(target)
	if xPlayer then
...
AddEventHandler('metaverse_licences:checkLicense', function(target, licenseType, cb)
	local xPlayer = ESX.GetPlayerFromId(target)
	if xPlayer then
...
AddEventHandler('metaverse_licences:getLicensesList', function(cb)
	GetLicensesList(cb)
end)

ESX.RegisterServerCallback('metaverse_licences:getLicense', function(source, cb, licenseType)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer then
		GetLicense(licenseType, cb)
	end
...
ESX.RegisterServerCallback('metaverse_licences:getLicenses', function(source, cb, target)
	local xPlayer = ESX.GetPlayerFromId(target)
	if xPlayer then
		GetLicenses(xPlayer.getIdentifier(), cb)
	end
...
ESX.RegisterServerCallback('metaverse_licences:checkLicense', function(source, cb, target, licenseType)
	local xPlayer = ESX.GetPlayerFromId(target)
	if xPlayer then
		CheckLicense(xPlayer.getIdentifier(), licenseType, cb)
	end
...
ESX.RegisterServerCallback('metaverse_licences:getLicensesList', function(source, cb)
	GetLicensesList(cb)
end)