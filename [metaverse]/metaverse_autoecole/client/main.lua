...
function StopTheoryTest(success)
	...
	if success then
		TriggerServerEvent('metaverse_autoecole:addLicense', 'dmv')
...
function StopDriveTest(success)
	if success then
		TriggerServerEvent('metaverse_autoecole:addLicense', CurrentTestType)
		ESX.ShowNotification(TranslateCap('passed_test'))
	else
		ESX.ShowNotification(TranslateCap('failed_test'))
	end
...
function OpenDMVSchoolMenu()
	...
	ESX.OpenContext("right", elements, function(menu,element)
		if element.value == "theory_test" then
			ESX.TriggerServerCallback('metaverse_autoecole:canYouPay', function(haveMoney)
				if haveMoney then
					ESX.CloseContext()
					StartTheoryTest()
				else
...
AddEventHandler('metaverse_autoecole:hasEnteredMarker', function(zone)
...
AddEventHandler('metaverse_autoecole:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.CloseContext()
end)

RegisterNetEvent('metaverse_autoecole:loadLicenses')
AddEventHandler('metaverse_autoecole:loadLicenses', function(licenses)
	Licenses = licenses
end)
...
CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('metaverse_autoecole:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('metaverse_autoecole:hasExitedMarker', LastZone)
		end
		Wait(sleep)
	end
end)
