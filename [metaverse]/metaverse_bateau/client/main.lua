				...
				ESX.Game.SpawnLocalVehicle(element.model, shop.Inside, shop.Inside.w, function (vehicle)
					...
					ESX.OpenContext("right", elements3, function(menu3,element3)
						if element3.value == "buy" then
							local plate = exports['metaverse_vehicleshop']:GeneratePlate()
							local vehicle = GetVehiclePedIsIn(playerPed, false)
							local props = ESX.Game.GetVehicleProperties(vehicle)
							props.plate = plate

							ESX.TriggerServerCallback('metaverse_bateau:buyBoat', function(bought)
								if bought then
...
function OpenBoatGarage(garage)
	ESX.TriggerServerCallback('metaverse_bateau:getGarage', function (ownedBoats)
		if #ownedBoats == 0 then
			...
			ESX.OpenContext("right", elements, function(menu,element)
				...
				if ESX.Game.IsSpawnPointClear(garage.SpawnPoint, 4.0) then
					TriggerServerEvent('metaverse_bateau:takeOutVehicle', vehicleProps.plate)
...
function OpenLicenceMenu(shop)
	local elements = {
		...
		{icon = "fas fa-ship", title = "Achat License Bateau"}
	}
	...
	ESX.OpenContext("right", elements, function(menu,element)
		...
		ESX.OpenContext("right", elements2, function(menu2,element2)
			...
			ESX.TriggerServerCallback('metaverse_bateau:buyBoatLicense', function (boughtLicense)
...
function StoreBoatInGarage(vehicle, teleportCoords)
	...
	ESX.TriggerServerCallback('metaverse_bateau:storeVehicle', function (rowsChanged)
