			...
			if IsControlJustReleased(0, 38) then
				if CurrentAction == 'boat_shop' then
					if not Config.LicenseEnable then
						...
						ESX.TriggerServerCallback('metaverse_licenses:CheckLicense', function(hasBoatLicense)
...
AddEventHandler('metaverse_bateau:hasEnteredMarker', function(zone, zoneNum)
...
AddEventHandler('metaverse_bateau:hasExitedMarker', function()
...
CreateThread(function()
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastZone ~= currentZone or LastZoneNum ~= currentZoneNum)) then
			...
			then
				TriggerEvent('metaverse_bateau:hasExitedMarker', LastZone)
				hasExited = true
			end
			...
			TriggerEvent('metaverse_bateau:hasEnteredMarker', currentZone, currentZoneNum)
		end

		if not hasExited and not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('metaverse_bateau:hasExitedMarker')
		end
