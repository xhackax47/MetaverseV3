...
function StopNPCJob(cancel)
	...
	if cancel then
		...
		TriggerServerEvent('metaverse_mecanique:onNPCJobCompleted')
	end
end
	...
	ESX.OpenContext("right", elements, function(menu,element)
		if element.value == 'vehicle_list' then
			if Config.EnableSocietyOwnedVehicles then
				...
				ESX.TriggerServerCallback('metaverse_society:getVehiclesInGarage', function(vehicles)
					...
					ESX.OpenContext("right", elements2, function(menu2,element2)
						...
						TriggerServerEvent('metaverse_society:removeVehicleFromGarage', 'mechanic', vehicleProps)
					end)
				end, 'mechanic')
			else
				local elements2 = {
		...
		elseif element.value == 'boss_actions' then
			TriggerEvent('metaverse_society:openBossMenu', 'mechanic', function(data, menu)
			      ESX.CloseContext()
			end)
		end
	end, function(menu)
...
function OpenMechanicHarvestMenu()
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'recrue' then
		local elements = {
			{unselectable = true, icon = "fas fa-gear", title = "Menu Récolte Mécano"},
		...
		ESX.OpenContext("right", elements, function(menu,element)
			if element.value == 'gaz_bottle' then
				TriggerServerEvent('metaverse_mecanique:startHarvest')
			elseif element.value == 'fix_tool' then
				TriggerServerEvent('metaverse_mecanique:startHarvest2')
			elseif element.value == 'caro_tool' then
				TriggerServerEvent('metaverse_mecanique:startHarvest3')
			end
		end, function(menu)
...
function OpenMechanicCraftMenu()
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'recrue' then
		local elements = {
			{unselectable = true, icon = "fas fa-gear", title = "Menu Craft Mécano"},
		...
		ESX.OpenContext("right", elements, function(menu,element)
			if element.value == 'blow_pipe' then
				TriggerServerEvent('metaverse_mecanique:startCraft')
			elseif element.value == 'fix_kit' then
				TriggerServerEvent('metaverse_mecanique:startCraft2')
			elseif element.value == 'caro_kit' then
				TriggerServerEvent('metaverse_mecanique:startCraft3')
			end
		end, function(menu)
	...
	ESX.OpenContext("right", elements, function(menu,element)
		...
		if element.value == "billing" then
			local elements2 = {
				...
				{title = "Montant", input = true, inputType = "number", inputMin = 1, inputMax = 250000, inputPlaceholder = "Montant à facturer.."},
				{icon = "fas fa-check-double", title = "Confirmer", value = "confirm"}
			}
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if amount == nil or amount < 0 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						ESX.CloseContext()
						TriggerServerEvent('metaverse_factures:sendBill', GetPlayerServerId(closestPlayer), 'society_mechanic', TranslateCap('mechanic'), amount)
					end
				end
			end)
					...
					if NPCOnJob then
						if NPCTargetDeleterZone then
							...
							if CurrentlyTowedVehicle == NPCTargetTowable then
								...
								TriggerServerEvent('metaverse_mecanique:onNPCJobMissionCompleted')
								StopNPCJob()
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('metaverse_mecanique:getStockItems', function(items)
		local elements = {
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			local elements2 = {
				...
				{title = "Montant", input = true, inputType = "number", inputMin = 1, inputMax = 100, inputPlaceholder = "Montant à retirer.."},
				{icon = "fas fa-check-double", title = "Confirmer", value = "confirm"}
			}
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if count == nil then
					...
					ESX.CloseContext()
					TriggerServerEvent('metaverse_mecanique:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('metaverse_mecanique:getPlayerInventory', function(inventory)
		local elements = {
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			local elements2 = {
				{unselectable = true, icon = "fas fa-box", title = element.title},
				{title = "Montant", input = true, inputType = "number", inputMin = 1, inputMax = 100, inputPlaceholder = "Montant à déposer.."},
				{icon = "fas fa-check-double", title = "Confirmer", value = "confirm"}
			}
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if count == nil then
					...
					ESX.CloseContext()
					TriggerServerEvent('metaverse_mecanique:putStockItems', itemName, count)
...
RegisterNetEvent('metaverse_mecanique:onHijack')
AddEventHandler('metaverse_mecanique:onHijack', function()
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed)
...
RegisterNetEvent('metaverse_mecanique:onCarokit')
AddEventHandler('metaverse_mecanique:onCarokit', function()
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed)
...
RegisterNetEvent('metaverse_mecanique:onFixkit')
AddEventHandler('metaverse_mecanique:onFixkit', function()
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed)
...
AddEventHandler('metaverse_mecanique:hasEnteredMarker', function(zone)
...
AddEventHandler('metaverse_mecanique:hasExitedMarker', function(zone)
	if zone =='VehicleDelivery' then
		NPCTargetDeleterZone = false
	elseif zone == 'Craft' then
		TriggerServerEvent('metaverse_mecanique:stopCraft')
		TriggerServerEvent('metaverse_mecanique:stopCraft2')
		TriggerServerEvent('metaverse_mecanique:stopCraft3')
	elseif zone == 'Garage' then
		TriggerServerEvent('metaverse_mecanique:stopHarvest')
		TriggerServerEvent('metaverse_mecanique:stopHarvest2')
		TriggerServerEvent('metaverse_mecanique:stopHarvest3')
	end
...
AddEventHandler('metaverse_mecanique:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()
...
AddEventHandler('metaverse_mecanique:hasExitedEntityZone', function(entity)
	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('metaverse_mecanique:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('metaverse_mecanique:hasExitedMarker', LastZone)
			end
		end
	Wait(Sleep)
	end
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('metaverse_mecanique:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity then
				TriggerEvent('metaverse_mecanique:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
	...
	local sleep = 500
		if CurrentAction then
			...
			if Config.EnableSocietyOwnedVehicles then
				...
				TriggerServerEvent('metaverse_society:putVehicleInGarage', 'mechanic', vehicleProps)
			else
...
RegisterKeyMapping('mechanicMenu', 'Ouvrir menu job Mécano', 'keyboard', 'F6')
RegisterKeyMapping('mechanicjob', 'Lancer les missions PNJ', 'keyboard', 'F6')
