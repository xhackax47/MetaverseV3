...
if Config.EnablePeds then
    AddEventHandler('esx:playerLoaded', function(playerId)
        TriggerClientEvent('metaverse_banque:pedHandler', playerId, netIdTable)
    end)
end

-- event
RegisterServerEvent('metaverse_banque:doingType')
AddEventHandler('metaverse_banque:doingType', function(typeData)
    ...
    if amount == nil or (not typeData.pincode and amount <= 0) then
        ...
        TriggerClientEvent("metaverse_banque:updateMoneyInUI", source, key, bankMoney, money)
    end
end)

-- register callbacks
ESX.RegisterServerCallback("metaverse_banque:getPlayerData", function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    local identifier = xPlayer.getIdentifier()
...
ESX.RegisterServerCallback("metaverse_banque:checkPincode", function(source, cb, inputPincode)
    local xPlayer = ESX.GetPlayerFromId(source)
    local identifier = xPlayer.getIdentifier()
...
RegisterServerEvent('metaverse_banque:logTransaction')
AddEventHandler('metaverse_banque:logTransaction', function(label,key,amount)
...
BANK = {
    CreatePeds = function()
        ...
        Wait(100)
        TriggerClientEvent('metaverse_banque:PedHandler', -1, netIdTable)
    end,
    DeletePeds = function()
