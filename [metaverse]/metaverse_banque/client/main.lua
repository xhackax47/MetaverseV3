...
--Functions
    ...
    function BANK:HandleUi(state, atm)
        ...
        ESX.TriggerServerCallback('metaverse_banque:getPlayerData', function(data)
            SendNUIMessage({
                showMenu = true,
                openATM = atm,
                datas = {
...
-- Events
RegisterNetEvent('metaverse_banque:closebanking', function()
    BANK:HandleUi(false)
end)

RegisterNetEvent('metaverse_banque:pedHandler', function(netIdTable)
    for i = 1, #netIdTable do
        BANK:LoadNpc(i, netIdTable[i])
    end
end)

RegisterNetEvent('metaverse_banque:updateMoneyInUI', function(doingType, bankMoney, money)
    SendNUIMessage({
        updateData = true,
        data = {
            type = doingType,
...
RegisterNUICallback('clickButton', function(data, cb)
    ...
    TriggerServerEvent("metaverse_banque:doingType", data)
    cb('ok')
end)
...
RegisterNUICallback('checkPincode', function(data, cb)
    ...
    ESX.TriggerServerCallback("metaverse_banque:checkPincode", function(pincode)
        if pincode then
            cb({
                success = true
            })
