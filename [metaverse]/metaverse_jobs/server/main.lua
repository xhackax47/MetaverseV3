...
RegisterServerEvent('metaverse_jobs:startWork', function(zoneIndex, zoneKey)
...
RegisterServerEvent('metaverse_jobs:stopWork', function()
	if playersWorking[source] then
		playersWorking[source] = nil
	end
end)

RegisterNetEvent('metaverse_jobs:caution', function(cautionType, cautionAmount, spawnPoint, vehicle)
	local xPlayer = ESX.GetPlayerFromId(source)

	if cautionType == 'take' then
		if cautionAmount <= Config.MaxCaution and cautionAmount >= 0 then
			TriggerEvent('metaverse_addonaccount:getAccount', 'caution', xPlayer.identifier, function(account)
				if xPlayer.getAccount('bank').money >= cautionAmount then
					...
					TriggerClientEvent('metaverse_jobs:spawnJobVehicle', xPlayer.source, spawnPoint, vehicle)
				else
