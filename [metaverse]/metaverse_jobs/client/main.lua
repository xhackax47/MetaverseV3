...
function OpenMenu()
	local elements = {
		...
		{icon = "fas fa-shirt", title = _U('job_wear'), value = "job_wear"},
...
AddEventHandler('metaverse_jobs:action', function(job, zone, zoneKey)
	menuIsShowed = true
	...
	elseif zone.Type == "work" then
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			TriggerServerEvent('metaverse_jobs:startWork', zone.Item, zoneKey)
		end
							...
							if playerPed == driverPed then
								...
								for i=1, #myPlate, 1 do
									if myPlate[i] == plate then
										...
										TriggerServerEvent('metaverse_jobs:caution', "give_back", giveBack, 0, 0)
...
AddEventHandler('metaverse_jobs:hasExitedMarker', function(zone)
	TriggerServerEvent('metaverse_jobs:stopWork')
	menuIsShowed = false
	isInMarker = false
end)
...
function spawnVehicle(spawnPoint, vehicle, vehicleCaution)
	hintIsShowed = false
	TriggerServerEvent('metaverse_jobs:caution', 'take', vehicleCaution, spawnPoint, vehicle)
end

RegisterNetEvent('metaverse_jobs:spawnJobVehicle',function(spawnPoint, vehicle)
	local playerPed = PlayerPedId()
	...
	ESX.Game.SpawnVehicle(vehicle.Hash, spawnPoint.Pos, spawnPoint.Heading, function(spawnedVehicle)
		...
		-- save & set plate
		local plate = 'META' .. math.random(100, 900)
		...
		-- A TESTER
		--['metaverse_vehlock']:giveKeys(playerPed, false)
			...
			if zones then
				...
				if IsControlJustReleased(0, 38) and not menuIsShowed and isInMarker then
					if onDuty or zone.Type == "cloakroom" then
						TriggerEvent('metaverse_jobs:action', job, zone, currentZone)
					end
				end
				...
				if not isInMarker and hasAlreadyEnteredMarker then
					...
					TriggerEvent('metaverse_jobs:hasExitedMarker', zone)
				end
			end
		end
	Wait(Sleep)
