...
RegisterNetEvent('metaverse_tv:update')
AddEventHandler('metaverse_tv:update', function(players)
...
CreateThread(function()
    ...
    TriggerServerEvent('metaverse_tv:fetch')
...
RegisterCommand('tv', function(source, args)
    if args[1] then
        for k, v in pairs(Config['Objects']) do
            ...
            if DoesEntityExist(obj) then
                TriggerServerEvent('metaverse_tv:add', args[1], v['Object'], GetEntityCoords(obj), v['Scale'], v['Offset'])
                break
            end
        end
    end
            ...
            if volume >= 0 then
                ...
                for k, v in pairs(data) do
                    if #(GetEntityCoords(PlayerPedId()) - v['Coords']) <= 5.0 then
                        ...
                        TriggerServerEvent('metaverse_tv:setvolume', k, volume)
                        break
                    end
                end
...
RegisterCommand('destroy', function(src, args)
    for k, v in pairs(data) do
        if #(GetEntityCoords(PlayerPedId()) - v['Coords']) <= 5.0 then
            TriggerServerEvent('metaverse_tv:destroy', k)
            break
        end
    end
end)

RegisterNetEvent('metaverse_tv:delete')
AddEventHandler('metaverse_tv:delete', function(id)
    if data[id] then
        if data[id]['DUI'] then
            DestroyDui(data[id]['DUI']['Long'])
        end
...
RegisterNetEvent('metaverse_tv:updatevolume')
AddEventHandler('metaverse_tv:updatevolume', function(id, volume)
    if data[id] then
