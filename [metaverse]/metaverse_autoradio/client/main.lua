...
RegisterNUICallback("action", function(data)
	...
	elseif data.action == "play" then
		if xSound:soundExists(nameid) then
			if xSound:isPaused(nameid) then
				TriggerServerEvent("metaverse_autoradio:ChangeState", true, nameid)
				local esperar = 0
				while nuiaberto do
					Wait(1000)
	...
	elseif data.action == "pause" then
		if xSound:soundExists(nameid) then
			if xSound:isPlaying(nameid) then
				TriggerServerEvent("metaverse_autoradio:ChangeState", false, nameid)
			end
		end
	...
	elseif data.action == "loop" then
		if xSound:soundExists(nameid) then
			...
			TriggerServerEvent("metaverse_autoradio:ChangeLoop",nameid,datasoundinfo.loop)
		else
	...
	elseif data.action == "forward" then
		if xSound:soundExists(nameid) then
			TriggerServerEvent("metaverse_autoradio:ChangePosition", 10, nameid)
		end
...
function ApplySound(quanti,plate)
	...
	if vadi <= 1.01 and vadi >= -0.001 and exis then
		...
		TriggerServerEvent("metaverse_autoradio:ChangeVolume", quanti, plate)
	end
end
...
function SetUrl(url,nid)
	...
	if url then
		...
		if encontrad then
			...
			TriggerServerEvent("metaverse_autoradio:ModifyURL",vehdata)
		else
			if IsPedInAnyVehicle(PlayerPedId(), false) then
				local veh = GetVehiclePedIsIn(PlayerPedId(),false)
				...
				vehdata.loop = datasoundinfo.loop
				TriggerServerEvent("metaverse_autoradio:AddVehicle",vehdata)
			end
		end
	else
...
if Config.ItemInVehicle then
	RegisterNetEvent("metaverse_autoradio:ShowNui")
	AddEventHandler("metaverse_autoradio:ShowNui", function()
		show()
	end)
end
...
function show(nomecenas)
    ...
    elseif nuiaberto then
		...
		ESX.ShowNotification("Tu ne peux pas faire ça maintenant.", 'error')
	end
end
...
AddEventHandler("metaverse_autoradio:AddVehicle", function(data)
...
AddEventHandler("metaverse_autoradio:ChangeState", function(tipo, nome)
...
AddEventHandler("metaverse_autoradio:ChangeLoop", function(tipo, nome)
	if xSound:soundExists(nome) then
...
AddEventHandler("metaverse_autoradio:ChangeVolume", function(som, range, nome)
	local carroe
	local crds
    for i = 1, #Zones do
        local v = Zones[i]
...
RegisterNetEvent("metaverse_autoradio:SendData")
AddEventHandler("metaverse_autoradio:SendData", function(data)
    Zones = data
    for i = 1, #Zones do
		local v = Zones[i]
		if v.isplaying then
