...
RegisterServerEvent('metaverse_crafting:getPlayerInventory')
AddEventHandler('metaverse_crafting:getPlayerInventory', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	if xPlayer ~= nil then
		TriggerClientEvent('metaverse_crafting:openMenu', _source, xPlayer.inventory)
	end
end)
...
RegisterServerEvent('metaverse_crafting:craftItem')
AddEventHandler('metaverse_crafting:craftItem', function(ingredients)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local item = findRecipe(ingredients)
	if not item then
		TriggerClientEvent('esx:showNotification', _source, 'Aucune recette trouvée avec ces ingrédients')
	else
		if xPlayer ~= nil then
			if hasAllIngredients(xPlayer.inventory, Config.Recipes[item]) then
				...
				TriggerClientEvent('esx:showNotification', _source, '~y~Objet fabriqué:~w~' .. itemLabel(item, xPlayer.inventory))
			else
				TriggerClientEvent('esx:showNotification', _source, 'Vous n\'avez pas tous les ingrédients')
			end
		end
	end
end)
