...
local function craftItem(ingredients)
	...
	TriggerServerEvent('metaverse_crafting:craftItem', ingredientsPrepped)
end

RegisterNetEvent('metaverse_crafting:openMenu')
AddEventHandler('metaverse_crafting:openMenu', function(playerInventory)
	...
	Citizen.CreateThread(function()
		while true do
			...
			if IsControlJustReleased(1, Config.Keyboard.keyCode) and GetLastInputMethod(2) then
				TriggerServerEvent('metaverse_crafting:getPlayerInventory')
			end
		end
	end)
end
		...
		while true do
			...
			if inDrawingRange then
				...
				if not display and isPlayerInRange(GetEntityCoords(PlayerPedId()), Config.Shop.shopCoordinates, Config.Shop.zoneSize.x) then
					...
					AddTextComponentString("Appuyer sur ~INPUT_CONTEXT~ pour fabriquer un objet")
