...
function OpenBuyLicenseMenu(zone)
    ...
    ESX.OpenContext(Config.MenuPosition, Elements, function(menu, element)
        if element.value == "buylicense" then
			ESX.TriggerServerCallback('metaverse_weaponshop:buyLicense', function(bought)
                if bought then
                    ESX.CloseContext()
                end
            end)
...
function OpenShopMenu(zone)
    ...
    ESX.OpenContext(Config.MenuPosition, Elements, function(menu, element)
        ...
        if element.value == "buy" then
            ESX.TriggerServerCallback('metaverse_weaponshop:buyWeapon', function(bought)
                if bought then
        ...
        for k, v in pairs(Config.Zones) do
            for i = 1, #v.Locations, 1 do
                    ...
                    if #(coords - CurrentShop) < 2.0 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            if Config.LicenseEnable and v.Legal then
                                ESX.TriggerServerCallback('metaverse_licences:checkLicense', function(hasWeaponLicense)
                ...
                if v.Legal then
                    ...
                    if pedMexicain < 1 then
                        Citizen.Wait(0)
                        local pedModel = 'u_m_y_mani'
                        while not HasModelLoaded(pedModel) do
                            RequestModel(GetHashKey(pedModel))
                            Wait(100)
                        end
                        local npc = CreatePed(4, GetHashKey(pedModel), v.Locations[i].x, v.Locations[i].y, v.Locations[i].z, 330.14, false, true)
                    
                        SetPedDiesWhenInjured(npc, false)
                        SetEntityInvincible(npc , true)
                        FreezeEntityPosition(npc, true)
                        SetBlockingOfNonTemporaryEvents(npc, true)
                        GiveWeaponToPed(npc, GetHashKey("WEAPON_TACTICALRIFLE"), 250, false, true)
                        SetCurrentPedWeapon(npc, GetHashKey("WEAPON_TACTICALRIFLE"), true)

                        pedMexicain = pedMexicain + 1			
                        Citizen.Wait(500)
                    end
                end
            end
