...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(ped, false) then
			TriggerServerEvent('metaverse_lockvehicules:check', GetVehicleNumberPlateText(vehicle))
			Wait(Config.checkKeyInterval)
		end
	end
end)

RegisterNetEvent('metaverse_lockvehicules:check')
AddEventHandler('metaverse_lockvehicules:check', function(keys, searched)
	hasKeys = keys
...
RegisterCommand('givekey', function(source, args)
	...
	ESX.TriggerServerCallback('metaverse_lockvehicules:getKeys', function(keys)
		if keys then
			...
			if distance < Config.key.PlayerRadius then
				...
				TriggerServerEvent('metaverse_lockvehicules:giveKeyCommand', GetPlayerServerId(target), plate)
				if Config.ESX_Notify then
			...
			if not isHotwire and not isSearch then
				...
				if IsControlJustPressed(1, Config.noKeys.SearchKey) and not hasSearched then
					...
					if chance == 1 then
						...
						TriggerServerEvent('metaverse_lockvehicules:setVehicleSearched', plate)
						TriggerServerEvent('metaverse_lockvehicules:givePlayerKey', plate)
						isSearch = false
					else
						isSearch = true
						if Config.ESX_ProgressBar then
					...
					TriggerServerEvent('metaverse_lockvehicules:check', plate)
				end
...
RegisterNetEvent('metaverse_lockvehicules:lockpick')
AddEventHandler('metaverse_lockvehicules:lockpick', function(shouldBreak)
	if hasKeys then
		return
	end
	if shouldBreak then
...
RegisterNetEvent('metaverse_lockvehicules:giveKeyCommand')
AddEventHandler('metaverse_lockvehicules:giveKeyCommand', function(plate)
	givePlayerKeys(plate)
	if Config.ESX_Notify then
...
RegisterNetEvent('metaverse_lockvehicules:sendNotify')
AddEventHandler('metaverse_lockvehicules:sendNotify', function(text, time, type)
	if Config.ESX_Notify then
...
function takePlayerKeys(plate)
	TriggerServerEvent('metaverse_lockvehicules:takePlayerKey', plate)
	TriggerServerEvent('metaverse_lockvehicules:check', plate)
end
...
Citizen.CreateThread(function()
	TriggerEvent('chat:addSuggestion', '/givekey',  'Donner vos clés au joueur le plus proche', {
