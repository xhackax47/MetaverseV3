...
function OpenPetMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pet_menu', {
		...
		if data.current.value == 'come_animal' and come == 0 then
			ESX.TriggerServerCallback('metaverse_animaux:getPet', function(pet)
				if pet == 'chien' then
					model = -1788665315
					come = 1
					openchien()
				...
				elseif pet == 'rottweiler' then
					...
					print('metaverse_animaux: unknown pet ' .. pet)
				end
			end)
			menu.close()
			...
			if distance < 5 then
				if count >= 1 then
					if status < 100 then
						...
						TriggerServerEvent('metaverse_animaux:consumePetFood')
						if status > 100 then
							status = 100
						end
						menu.close()
...
Citizen.CreateThread(function()
	while true do
		...
		if status == 0 then
			TriggerServerEvent('metaverse_animaux:petDied')
			DeleteEntity(ped)
...
function OpenPetShop()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pet_shop', {
		...
		ESX.TriggerServerCallback('metaverse_animaux:buyPet', function(boughtPed)
			if boughtPed then
				menu.close()
			end
