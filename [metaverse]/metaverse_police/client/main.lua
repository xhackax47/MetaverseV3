			...
			if Config.EnableESXService then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerEvent('metaverse_police:updateBlip')
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'handcuff' then
						TriggerServerEvent('metaverse_police:handcuff', GetPlayerServerId(closestPlayer))
					elseif action == 'drag' then
						TriggerServerEvent('metaverse_police:drag', GetPlayerServerId(closestPlayer))
					elseif action == 'put_in_vehicle' then
						TriggerServerEvent('metaverse_police:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif action == 'out_the_vehicle' then
						TriggerServerEvent('metaverse_police:OutVehicle', GetPlayerServerId(closestPlayer))
...
function OpenIdentityCardMenu(player)
	ESX.TriggerServerCallback('metaverse_police:getOtherPlayerData', function(data)
		local elements = {
...
function OpenBodySearchMenu(player)
	...
	ESX.TriggerServerCallback('metaverse_police:getOtherPlayerData', function(data)
		...
		for i=1, #data.weapons, 1 do
			elements[#elements+1] = {
				icon = "fas fa-gun",
				title    = TranslateCap('confiscate_weapon', data.weapons[i].label, data.weapons[i].ammo),
				value    = data.weapons[i].weapon,
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			if data.current.value then
				TriggerServerEvent('metaverse_police:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
...
function OpenFineCategoryMenu(player, category)
	ESX.TriggerServerCallback('metaverse_police:getFineList', function(fines)
		local elements = {
			{unselectable = true, icon = "fas fa-scroll", title = TranslateCap('fine')}
		}
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('metaverse_factures:sendBill', GetPlayerServerId(player), 'society_police', TranslateCap('fine_total', data.current.fineLabel), data.current.amount)
			else
				TriggerServerEvent('metaverse_factures:sendBill', GetPlayerServerId(player), '', TranslateCap('fine_total', data.current.fineLabel), data.current.amount)
			end
...
function LookupVehicle(elementF)
	...
	ESX.OpenContext("right", elements, function(menu,element)
		...
		if not data.value or length < 2 or length > 8 then
			...
			ESX.TriggerServerCallback('metaverse_police:getVehicleInfos', function(retrivedInfo)
				local elements = {
					{unselectable = true, icon = "fas fa-car", title = element.title},
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('metaverse_police:getOtherPlayerData', function(playerData)
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			TriggerServerEvent('metaverse_police:message', GetPlayerServerId(player), TranslateCap('license_revoked', data.current.label))

			TriggerServerEvent('metaverse_licenceslicense:removeLicense', GetPlayerServerId(player), data.current.type)
...
function OpenUnpaidBillsMenu(player)
	...
	ESX.TriggerServerCallback('metaverse_factures:getTargetBills', function(bills)
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('metaverse_police:getArmoryWeapons', function(weapons)
		local elements = {
		...
		for i=1, #weapons, 1 do
			if weapons[i].count > 0 then
				elements[#elements+1] = {
					icon = "fas fa-gun",
					title = 'x' .. weapons[i].count .. ' ' .. weapons[i].weapon,
					value = weapons[i].weapon
				}
			end
		end
		...
		ESX.OpenContext("right", elements, function(menu,element)
			local data = {current = element}
			ESX.TriggerServerCallback('metaverse_police:removeArmoryWeapon', function()
				ESX.CloseContext()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	for i=1, #weaponList, 1 do
		local weaponHash = joaat(weaponList[i].weapon)

		if HasPedGotWeapon(playerPed, weaponHash, false) and weaponList[i].weapon ~= 'WEAPON_UNARMED' then
			elements[#elements+1] = {
				icon = "fas fa-gun",
				title = weaponList[i].label,
				value = weaponList[i].weapon
			}
		end
	end
	...
	ESX.OpenContext("right", elements, function(menu,element)
		local data = {current = element}
		ESX.TriggerServerCallback('metaverse_police:addArmoryWeapon', function()
			ESX.CloseContext()
			OpenPutWeaponMenu()
...
function OpenBuyWeaponsMenu()
	...
	for k,v in ipairs(Config.AuthorizedWeapons[ESX.PlayerData.job.grade_name]) do
		local weaponNum, weapon = v.weapon
		...
		if hasWeapon and v.components then
			label = ('%s: <span style="color:green;">></span>'):format(v.label)
		elseif hasWeapon and not v.components then
			label = ('%s: <span style="color:green;">%s</span>'):format(v.label, TranslateCap('armory_owned'))
		else
			if v.price > 0 then
				label = ('%s: <span style="color:green;">%s</span>'):format(v.label, TranslateCap('armory_item', ESX.Math.GroupDigits(v.price)))
			else
				label = ('%s: <span style="color:green;">%s</span>'):format(v.label, TranslateCap('armory_free'))
			end
		end
	...
	ESX.OpenContext("right", elements, function(menu,element)
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('metaverse_police:buyWeapon', function(bought)
				if bought then
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('metaverse_police:getStockItems', function(items)
		local elements = {
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if not count then
					...
					ESX.CloseContext()
					TriggerServerEvent('metaverse_police:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('metaverse_police:getPlayerInventory', function(inventory)
		local elements = {
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if not count then
					...
					ESX.CloseContext()
					TriggerServerEvent('metaverse_police:putStockItems', itemName, count)
...
AddEventHandler('esx:setJob', function(job)
	...
	if job.name == 'police' then
		Wait(1000)
		TriggerServerEvent('metaverse_police:forceBlip')
	end
end)
...
AddEventHandler('metaverse_police:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('metaverse_police:hasExitedMarker', function(station, part, partNum)
...
AddEventHandler('metaverse_police:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()
...
AddEventHandler('metaverse_police:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('metaverse_police:handcuff')
AddEventHandler('metaverse_police:handcuff', function()
...
RegisterNetEvent('metaverse_police:unrestrain')
AddEventHandler('metaverse_police:unrestrain', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
		isHandcuffed = false
...
RegisterNetEvent('metaverse_police:drag')
AddEventHandler('metaverse_police:drag', function(copId)
	if isHandcuffed then
...
RegisterNetEvent('metaverse_police:putInVehicle')
AddEventHandler('metaverse_police:putInVehicle', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
...
RegisterNetEvent('metaverse_police:OutVehicle')
AddEventHandler('metaverse_police:OutVehicle', function()
	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('metaverse_police:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('metaverse_police:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
		...
		local Sleep = 1500
			...
			if closestDistance ~= -1 and closestDistance <= 3.0 then
				if LastEntity ~= closestEntity then
					TriggerEvent('metaverse_police:hasEnteredEntityZone', closestEntity)
					LastEntity = closestEntity
				end
			else
				if LastEntity then
					TriggerEvent('metaverse_police:hasExitedEntityZone', LastEntity)
					LastEntity = nil
				end
			end
		Wait(Sleep)
...
ESX.RegisterInput("police:interact", "(ESX PoliceJob) " .. TranslateCap('interaction'), "keyboard", "E", function()
	...
	elseif CurrentAction == 'menu_boss_actions' then
		ESX.CloseContext()
		TriggerEvent('metaverse_society:openBossMenu', 'police', function(data, menu)
			--menu.close()
...
RegisterNetEvent('metaverse_police:updateBlip')
AddEventHandler('metaverse_police:updateBlip', function()
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' then
		ESX.TriggerServerCallback('metaverse_society:getOnlinePlayers', function(players)
...
AddEventHandler('esx:onPlayerSpawn', function(spawn)
	isDead = false
	TriggerEvent('metaverse_police:unrestrain')

	if not hasAlreadyJoined then
		TriggerServerEvent('metaverse_police:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('metaverse_police:unrestrain')
		TriggerEvent('gcphone:removeSpecialContact', 'police')
