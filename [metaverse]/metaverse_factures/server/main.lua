RegisterServerEvent('metaverse_factures:sendBill')
AddEventHandler('metaverse_factures:sendBill', function(playerId, sharedAccountName, label, amount)
	local xPlayer = ESX.GetPlayerFromId(source)
	local xTarget = ESX.GetPlayerFromId(playerId)
	amount = ESX.Math.Round(amount)

	if amount > 0 and xTarget then
		TriggerEvent('metaverse_addonaccount:getSharedAccount', sharedAccountName, function(account)
			if account then
...
ESX.RegisterServerCallback('metaverse_factures:getBills', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('metaverse_factures:getTargetBills', function(source, cb, target)
...
ESX.RegisterServerCallback('metaverse_factures:payBill', function(source, cb, billId)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	function(result)
		if result then
			...
			if result.target_type == 'player' then
				...
				TriggerEvent('metaverse_addonaccount:getSharedAccount', result.target, function(account)
					if xPlayer.getMoney() >= amount then
						MySQL.update('DELETE FROM billing WHERE id = ?', {billId},
						function(rowsChanged)
							if rowsChanged == 1 then
