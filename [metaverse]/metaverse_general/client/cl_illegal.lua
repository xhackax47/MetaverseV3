...
function IsAbleToSteal(targetSID, err)
	ESX.TriggerServerCallback('metaverse_general:getValue', function(result)
		local result = result
...
function OpenStealMenu(target, target_id)
	...
	ESX.TriggerServerCallback('metaverse_general:getOtherPlayerData', function(data)
		local elements = {}
					...
					if data2.current.action == 'steal' then
						...
						if itemType == 'item_standard' then
							ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'steal_inventory_item_standard', {
								...
								TriggerServerEvent('metaverse_general:stealPlayerItem', GetPlayerServerId(target), itemType, itemName, quantity)
							...
							TriggerServerEvent('metaverse_general:stealPlayerItem', GetPlayerServerId(target), itemType, itemName, amount)
							OpenStealMenu(target)
						end
		...
		if canHandsUp then
			if IsControlJustReleased(0, Keys['X']) then
				...
				if handsup then
					...
					TriggerServerEvent('metaverse_general:update', handsup)
				else
					handsup = true
