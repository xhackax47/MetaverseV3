...
function OpenAmbulanceActionsMenu()
	...
	ESX.OpenContext("right", elements, function(menu, element)
		...
		elseif element.value == 'boss_actions' then
			TriggerEvent('metaverse_society:openBossMenu', 'ambulance', function(data, menu)
				menu.close()
			end, { wash = false })
		end
	end)
		...
		if element.value == "citizen_interaction" then
			...
			ESX.OpenContext("right", elements2, function(menu2, element2)
				...
				if element2.value == 'search' then
					TriggerServerEvent('metaverse_ambulance:svsearch')
				elseif closestPlayer == -1 or closestDistance > 1.0 then
					...
					elseif element2.value == 'small' then
						ESX.TriggerServerCallback('metaverse_ambulance:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									...
									TriggerServerEvent('metaverse_ambulance:removeItem', 'bandage')
									TriggerServerEvent('metaverse_ambulance:heal', GetPlayerServerId(closestPlayer), 'small')
						...
						ESX.TriggerServerCallback('metaverse_ambulance:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									...
									TriggerServerEvent('metaverse_ambulance:removeItem', 'medikit')
									TriggerServerEvent('metaverse_ambulance:heal', GetPlayerServerId(closestPlayer), 'big')
									ESX.ShowNotification(TranslateCap('heal_complete', GetPlayerName(closestPlayer)))
									isBusy = false
								else
									ESX.ShowNotification(TranslateCap('player_not_conscious'))
					...
					elseif element2.value == 'put_in_vehicle' then
						TriggerServerEvent('metaverse_ambulance:putInVehicle', GetPlayerServerId(closestPlayer))
					end
				end
			end)
		end
	...
	ESX.TriggerServerCallback('metaverse_ambulance:getItemAmount', function(quantity)
		if quantity > 0 then
			...
			if IsPedDeadOrDying(closestPlayerPed, 1) then
				...
				TriggerServerEvent('metaverse_ambulance:removeItem', 'medikit')
				TriggerServerEvent('metaverse_ambulance:revive', GetPlayerServerId(closestPlayer))
			else
	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or
				...
				then
					TriggerEvent('metaverse_ambulance:hasExitedMarker', LastHospital, LastPart, LastPartNum)
					hasExited = true
				end

				HasAlreadyEnteredMarker, LastHospital, LastPart, LastPartNum = true, currentHospital, currentPart, currentPartNum

				TriggerEvent('metaverse_ambulance:hasEnteredMarker', currentHospital, currentPart, currentPartNum)
			end
...
AddEventHandler('metaverse_ambulance:hasEnteredMarker', function(hospital, part, partNum)
...
AddEventHandler('metaverse_ambulance:hasExitedMarker', function(hospital, part, partNum)
...
AddEventHandler('metaverse_ambulance:putInVehicle', function()
	local playerPed = PlayerPedId()
	...
	ESX.OpenContext("right", elements, function(menu, element)
		...
		elseif element.value == "ambulance_wear" then
			ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
				...
				isOnDuty = true
				ESX.TriggerServerCallback('metaverse_ambulance:getDeadPlayers', function(_deadPlayers)
					TriggerEvent('metaverse_ambulance:setDeadPlayers', _deadPlayers)
				end)
				if Config.Debug then
...
function OpenPharmacyMenu()
	...
	ESX.OpenContext("right", elements, function(menu, element)
		...
		ESX.OpenContext("right", elements2, function(menu2, element2)
			...
			TriggerServerEvent('metaverse_ambulance:giveItem', element.item, amount)
		end, function(menu)
			OpenPharmacyMenu()
		end)
	end)
...
AddEventHandler('metaverse_ambulance:heal', function(healType, quiet)
	local playerPed = PlayerPedId()
...
RegisterNetEvent('metaverse_ambulance:PlayerDead')
AddEventHandler('metaverse_ambulance:PlayerDead', function(Player)
	if Config.Debug then
...
RegisterNetEvent('metaverse_ambulance:PlayerNotDead')
AddEventHandler('metaverse_ambulance:PlayerNotDead', function(Player)
...
RegisterNetEvent('metaverse_ambulance:setDeadPlayers')
AddEventHandler('metaverse_ambulance:setDeadPlayers', function(_deadPlayers)
...
RegisterNetEvent('metaverse_ambulance:PlayerDistressed')
AddEventHandler('metaverse_ambulance:PlayerDistressed', function(playerId, playerCoords)
