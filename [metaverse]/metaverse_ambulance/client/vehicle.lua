...
function OpenVehicleSpawnerMenu(type, hospital, part, partNum)
	...
	ESX.OpenContext("right", elements, function(menu,element)
		...
		elseif element.action == "garage" then
			...
			ESX.TriggerServerCallback('metaverse_vehicleshop:retrieveJobVehicles', function(jobVehicles)
							...
							if elementG.stored == 1 then
								...
								if foundSpawn then
									...
									ESX.Game.SpawnVehicle(elementG.model, spawnPoint.coords, spawnPoint.heading, function(vehicle)
										...
										TriggerServerEvent('metaverse_vehicleshop:setJobVehicleState', elementG.plate, false)
...
function StoreNearbyVehicle(playerCoords)
	...
	ESX.TriggerServerCallback('metaverse_ambulance:storeNearbyVehicle', function(plate)
		if plate then
...
function OpenShopMenu(elements, restoreCoords, shopCoords)
	...
	ESX.OpenContext("right", elements, function(menu,element)
		local elements2 = {
			...
			{icon = "fas fa-eye", title = "Prévisualisation", value = "view"}
		}
		...
		ESX.OpenContext("right", elements2, function(menu2,element2)
			if element2.value == "view" then
				...
				local elements3 = {
					{unselectable = true, icon = "fas fa-car", title = element.title},
					{icon = "fas fa-check-double", title = "Acheter", value = "buy"},
					{icon = "fas fa-eye", title = "Arrêter la prévisualisation", value = "stop"}
				}
				...
				ESX.OpenContext("right", elements3, function(menu3,element3)
					...
					elseif element3.value == "buy" then
						local newPlate = exports['metaverse_vehicleshop']:GeneratePlate()
						local vehicle  = GetVehiclePedIsIn(playerPed, false)
						local props    = ESX.Game.GetVehicleProperties(vehicle)
						props.plate    = newPlate

						ESX.TriggerServerCallback('metaverse_ambulance:buyJobVehicle', function (bought)
							if bought then
