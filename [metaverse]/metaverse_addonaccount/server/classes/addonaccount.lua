function CreateAddonAccount(name, owner, money)
	...
	function self.addMoney(m)
		...
		self.save()
		TriggerEvent('metaverse_addonaccount:addMoney', self.name, m)
	end

	function self.removeMoney(m)
		self.money = self.money - m
		self.save()
		TriggerEvent('metaverse_addonaccount:removeMoney', self.name, m)
	end

	function self.setMoney(m)
		self.money = m
		self.save()
		TriggerEvent('metaverse_addonaccount:setMoney', self.name, m)
	end
	...
	function self.save()
		...
		TriggerClientEvent('metaverse_addonaccount:setMoney', -1, self.name, self.money)
	end
