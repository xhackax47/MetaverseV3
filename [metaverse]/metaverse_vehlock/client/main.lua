...
-- Loop function
RegisterNetEvent('metaverse_vehlock:UpdateClientLocks', function(table)
...
function changeLock(plate, veh, isLockpick)
	lib.callback('metaverse_vehlock:getVehState', false, function(isLocked)
		if isLocked then -- is locked
			lib.callback('metaverse_vehlock:updateLock', false, function(result)
				if result then
...
function changeLockRadius()
	...
	if #vehs > 0 then
		for i=1, #vehs do
			lib.callback('metaverse_vehlock:isOwner', false, function(isOwner)
				if isOwner then
					changeLock(Trim(GetVehicleNumberPlateText(vehs[i].vehicle)), vehs[i].vehicle)
				else
					lib.callback('metaverse_vehlock:hasKey', false, function(hasKey)
						if hasKey then
							changeLock(Trim(GetVehicleNumberPlateText(vehs[i].vehicle)), vehs[i].vehicle)
						end
...
RegisterNetEvent('metaverse_vehlock:_lockVehicle', function ()
	if isOnTimer > 0 then
...
RegisterCommand('_lockVehicle', function()
	TriggerEvent('metaverse_vehlock:_lockVehicle')
end, false)
RegisterKeyMapping('_lockVehicle', langSettings[language]['keyMappingLabel'], 'keyboard', 'U')
...
-- GiveKeys
function giveKeys(target, ignoreFlags) -- ignoreFlags is default false, if you want to give keys to a player for a job car then use exports['metaverse_vehlock']:giveKeys(int playerId, bool ignoreFlags)
	...
	if IsPedInAnyVehicle(PlayerPedId(), false) then
		...
		if veh then
			...
			if dist <= 5 then
				lib.callback('metaverse_vehlock:isOwner', false, function(isOwner)
					if isOwner then
						lib.callback('metaverse_vehlock:giveKeys', false, function(success)
							if success then
...
RegisterNetEvent('metaverse_vehlock:giveKeys_TARGETONLY', function(targetParameter)
	lib.callback('metaverse_vehlock:isOwner', false, function(isOwner)
		if isOwner then
			...
			if targetFramework == 'ox' then
				...
				exports.ox_target:addGlobalPlayer({
					{
						name = 'metaverse_vehlock:targetsys2GiveKeys',
						...
						onSelect = function(entity)
							...
							lib.hideTextUI()
							exports.ox_target:removeGlobalPlayer('metaverse_vehlock:targetsys2GiveKeys')
						end,
						distance = 1.0
					}
				})
			...
			if targetSys2 then
				...
				targetSys2 = nil
				if usingTarget then
					if targetFramework == 'ox' then exports.ox_target:removeGlobalPlayer('metaverse_vehlock:targetsys2GiveKeys')
...
RegisterNetEvent('metaverse_vehlock:giveKeys',function(target)
	giveKeys(target)
end)

-- RemoveKeys
RegisterNetEvent('metaverse_vehlock:removeKeys', function(targetParameter) -- targetParameter = entity from target
	local veh
	if usingTarget then
		if IsPedInAnyVehicle(PlayerPedId(), false) then
			...
			lib.callback('metaverse_vehlock:isOwner', false, function(isOwner)
				if isOwner then
					local keys = lib.callback.await('metaverse_vehlock:getKeysOnPlate', false, Trim(GetVehicleNumberPlateText(veh)))
					local keysOnPlate = {}
					if #keys > 0 then
						for i=1, #keys do
							table.insert(keysOnPlate,
							{
								title=lib.callback.await('metaverse_vehlock:getPlayerNameFromIdentifier', false, keys[i].identifier),
								...
								serverEvent='metaverse_vehlock:removeKeyFromPlate',
							})
						end
					end
					if #keysOnPlate > 0 then
						lib.registerContext({
							id = 'metaverse_vehlock-removeKeys',
							title = Trim(GetVehicleNumberPlateText(veh)),
							options = {
								table.unpack(keysOnPlate)
							}
						})
						lib.showContext('metaverse_vehlock-removeKeys')
			...
			lib.callback('metaverse_vehlock:isOwner', false, function(isOwner)
				if isOwner then
					...
					if targetFramework == 'ox' then
						...
						exports.ox_target:addGlobalPlayer({
							{
								name = 'metaverse_vehlock:targetsys2RemKeys',
								icon = 'fa-solid fa-user-lock',
								...
								onSelect = function(entity)
									...
									targetSys2 = GetPlayerServerId(playerId)
									lib.hideTextUI()
									exports.ox_target:removeGlobalPlayer('metaverse_vehlock:targetsys2RemKeys')
								end,
								distance = 1.0
							}
						})
					...
					if targetSys2 then
						lib.callback('metaverse_vehlock:hasKey', false, function(hasKey)
							if hasKey then
								TriggerServerEvent('metaverse_vehlock:removeKeysID', Trim(GetVehicleNumberPlateText(targetParameter)), targetSys2)
							else
						...
						targetSys2 = nil
						if usingTarget then
							if targetFramework == 'ox' then exports.ox_target:removeGlobalPlayer('metaverse_vehlock:targetsys2RemKeys')
...
RegisterNetEvent('metaverse_vehlock:lockpickVehicle', function()
	if not IsPedInAnyVehicle(PlayerPedId(), true) then
		local veh = lib.getClosestVehicle(GetEntityCoords(PlayerPedId()), 2, false)
		if veh then
			lib.callback('metaverse_vehlock:lockpickRemove', false, function(removed)
				if removed then
					doLockpicking(veh)
				else
...
if usingTarget then
	if targetFramework == 'ox' then
		local toptions = {
			[2] = {
				name = 'metaverse_vehlock:giveKeysTarget',
				icon = 'fa-solid fa-user-lock',
				label = langSettings[language]['GiveKeys'],
				distance = 1.5,
				onSelect = function(data)
					TriggerEvent('metaverse_vehlock:giveKeys_TARGETONLY',data.entity)
				end
			},
			[3] = {
				name = 'metaverse_vehlock:removeKeysTarget',
				icon = 'fa-solid fa-user-lock',
				label = langSettings[language]['RemoveKeys'],
				distance = 1.5,
				onSelect = function(data)
					TriggerEvent('metaverse_vehlock:removeKeys',data.entity)
				end
			},
			[4] = {
				name = 'metaverse_vehlock:lockPick',
                icon = 'fa-solid fa-handcuffs',
                label = langSettings[language]['UseLockpick'],
                distance = 1.5,
                event = 'metaverse_vehlock:lockpickVehicle',
                items = 'lockpick'
			},
		}
		if lockpickEnabled then
			table.insert(toptions, 1, {
                name = 'metaverse_vehlock:lockVehicle',
		...
		function targetLockSys(veh) -- ox_target function only
			local result = false
            		local plate = Trim(GetVehicleNumberPlateText(veh))
           		result = lib.callback.await('metaverse_vehlock:isOwner', false, plate)
			if not result and enableKeys then
				result = lib.callback.await('metaverse_vehlock:hasKey', false, plate)
			end
			if result then
	...
	elseif targetFramework == 'qtarget' then
		local toptions = {
			...
			[3] = {
				...
				label = langSettings[language]['RemoveKeys'],
				action = function(entity)
					TriggerEvent('metaverse_vehlock:giveKeys_TARGETONLY',entity)
				end
			},
			[4] = {
				icon = 'fa-solid fa-user-lock',
				label = langSettings[language]['RemoveKeys'],
				action = function(entity)
					TriggerEvent('metaverse_vehlock:removeKeys',entity)
				end
			}
		}
		if lockpickEnabled then
