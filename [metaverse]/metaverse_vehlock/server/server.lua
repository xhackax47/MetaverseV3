...
lib.versionCheck('OppoNoppo/metaverse_vehlock')
...
lib.callback.register('metaverse_vehlock:getLocks', function()
    return getLocks()
end)

lib.callback.register('metaverse_vehlock:getVehState', function(source, plate)
    local r = false
...
lib.callback.register('metaverse_vehlock:updateLock', function(source, plate, state)
    local r = false
...
lib.callback.register('metaverse_vehlock:giveKeys', function(source, plate, target)
    local r = false
    local r2 = false
...
RegisterNetEvent('metaverse_vehlock:removeKeys', function(source, data)
...
RegisterNetEvent('metaverse_vehlock:removeKeysID', function(plate, playerId)
    MySQL.query('DELETE FROM `'..('%s'):format(dbTableKeys)..'` WHERE plate = ? AND identifier = ?', {plate, GetPlayerIden(playerId)})
end)

RegisterNetEvent('metaverse_vehlock:wipeKeys', function(plate)
...
RegisterNetEvent('metaverse_vehlock:wipeKeys:ALL', function()
    local r = MySQL.query('DELETE FROM `'..('%s'):format(dbTableKeys)..'`', {})
end)

lib.callback.register('metaverse_vehlock:hasKey', function(source, plate, identifier, isPlayerID)
    if isPlayerID then
...
lib.callback.register('metaverse_vehlock:getKeysOnPlate', function(source, plate)
...
RegisterNetEvent('metaverse_vehlock:removeKeyFromPlate', function(data)
    local plate = data.plate
    local identifier = data.identifier
    MySQL.query('DELETE FROM `'..('%s'):format(dbTableKeys)..'` WHERE plate = ? AND identifier = ?', {plate, identifier})
end)

lib.callback.register('metaverse_vehlock:getPlayerNameFromIdentifier', function(source,identifier)
...
lib.callback.register('metaverse_vehlock:isOwner', function(source, plate)
...
local function wipeLock(data)
    if data then
        ...
        if t then
            if consolePrinting then print(('[^6metaverse_vehlock^0] Plate: ^6%s^0 has been removed due to not being in the map anymore'):format(data)) end
        end
    end
end

local function checkLocks()
    if consolePrinting then print(('[^6metaverse_vehlock^0] Cleaning up database table `^6%s^0`'):format(dbTableLocks)) end
    ...
    if consolePrinting then print('[^6metaverse_vehlock^0] Database table cleanup has been completed. ^6'..#tempLockedVehs..'^0 plates have been removed.' ) end
end

CreateThread(function()
    while true do
        lockedVehicles = getLocks()
        TriggerClientEvent('metaverse_vehlock:UpdateClientLocks', -1, lockedVehicles)
...
if enableKeys then
    lib.addCommand(false, {'givekeys'}, function(source, args)
        TriggerClientEvent('metaverse_vehlock:giveKeys', source, args.target)
    end, {'target:number'})

    lib.addCommand(false, {'removeKeys'}, function(source)
        TriggerClientEvent('metaverse_vehlock:removeKeys', source)
    end)

    lib.addCommand('group.admin', {'wipekeys'}, function(source, args)
        TriggerEvent('metaverse_vehlock:wipeKeys', args.plate)
    end, {'plate:string'})
end
--[[
...
CreateThread(function()
    if wipeKeysOnStartup then
        TriggerEvent('metaverse_vehlock:wipeKeys:ALL')
    end
end)
...
if lockpickEnabled then
    if framework == 'esx' then
        _Framework.RegisterUsableItem('lockpick', function(source)
            TriggerClientEvent('metaverse_vehlock:lockpickVehicle', source)
        end)
    elseif framework == 'qb' then
        ...
        print('[^6metaverse_vehlock^0] Current framework setting does is not supported for lockpicking. The script will go further with lockpickEnabled set to ^6false^0.' )
        lockpickEnabled = false
    end
end

lib.callback.register('metaverse_vehlock:lockpickRemove', function(source)
    if framework == 'esx' then
