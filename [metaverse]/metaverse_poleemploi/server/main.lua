...
ESX.RegisterServerCallback('metaverse_poleemploi:getJobsList', function(source, cb)
  local jobs = getJobs()
  cb(jobs)
end)
...
RegisterServerEvent('metaverse_poleemploi:setJob')
AddEventHandler('metaverse_poleemploi:setJob', function(job)
  local source = source
  ...
  if xPlayer and IsNearCentre(source) and IsJobAvailable(job) then
    ...
    print("[^3WARNING^7] User ^5".. source .. "^7 Attempted to Exploit ^5`metaverse_poleemploi:setJob`^7!")
  end
end)
