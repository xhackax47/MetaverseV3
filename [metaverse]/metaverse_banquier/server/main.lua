TriggerEvent('gcphone:registerNumber', 'banker', _('phone_receive'), false, false)
TriggerEvent('metaverse_society:registerSociety', 'banker', TranslateCap('phone_label'), 'society_banker', 'society_banker', 'society_banker', {type = 'public'})

RegisterServerEvent('metaverse_banquier:customerDeposit')
AddEventHandler('metaverse_banquier:customerDeposit', function (target, amount)
	local xPlayer = ESX.GetPlayerFromId(target)

	TriggerEvent('metaverse_addonaccount:getSharedAccount', 'society_banker', function (account)
		if amount > 0 and account.money >= amount then
			account.removeMoney(amount)

			TriggerEvent('metaverse_addonaccount:getAccount', 'bank_savings', xPlayer.identifier, function (account)
...
RegisterServerEvent('metaverse_banquier:customerWithdraw')
AddEventHandler('metaverse_banquier:customerWithdraw', function (target, amount)
	local xPlayer = ESX.GetPlayerFromId(target)
...
ESX.RegisterServerCallback('metaverse_banquier:getCustomers', function (source, cb)
