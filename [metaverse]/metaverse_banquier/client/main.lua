...
function OpenBankActionsMenu()
	...
	ESX.OpenContext("right", elements, function(menu,element)
		...
		elseif element.value == "boss_actions" then
			TriggerEvent('metaverse_society:openBossMenu', 'banker', function(data, menu)
				menu.close()
			end, {wash = false})
...
function OpenCustomersMenu()
	ESX.TriggerServerCallback('metaverse_banquier:getCustomers', function(customers)
		local elements = {
					...
					ESX.OpenContext("right", elements, function(menu,element)
						if element.val == "confirm" then
							...
							if amount == nil then
								...
								ESX.CloseContext()
								TriggerServerEvent('metaverse_banquier:customerDeposit', customer.source, amount)
					...
					ESX.OpenContext("right", elements, function(menu,element)
						if element.val == "confirm" then
							...
							if amount == nil then
								...
								ESX.CloseContext()
								TriggerServerEvent('metaverse_banquier:customerWithdraw', customer.source, amount)
		...
		if element.val == "confirm" then
			...
			if amount == nil then
				...
				if closestPlayer == -1 or closestDistance > 5.0 then
					...
					TriggerServerEvent('metaverse_factures:sendBill', GetPlayerServerId(closestPlayer), 'society_banker', 'Bank', amount)
				end
			end
		end
	end, function(menu)
...
AddEventHandler('metaverse_banquier:hasEnteredMarker', function (zone)
...
AddEventHandler('metaverse_banquier:hasExitedMarker', function (zone)
	CurrentAction = nil
	ESX.CloseContext()
end)
	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'banker' then
			...
			if isInMarker and not hasAlreadyEnteredMarker then
				...
				TriggerEvent('metaverse_banquier:hasEnteredMarker', currentZone)
			end

			if not isInMarker and hasAlreadyEnteredMarker then
				hasAlreadyEnteredMarker = false
				TriggerEvent('metaverse_banquier:hasExitedMarker', lastZone)
			end
...
RegisterNetEvent('gchone:loaded')
