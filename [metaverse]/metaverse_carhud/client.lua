SetFlyThroughWindscreenParams((60 / 3.6), (70 / 3.6), 17.0, 2000);
...
RegisterCommand('leftIndicator', function()
    ...
    TriggerServerEvent('metaverse_carhud:syncIndicators', VehToNet(GetVehiclePedIsUsing(PlayerPedId())), 1)
end)

RegisterCommand('rightIndicator', function()
    if not IsPedInAnyVehicle(PlayerPedId()) then return end
    TriggerServerEvent('metaverse_carhud:syncIndicators', VehToNet(GetVehiclePedIsUsing(PlayerPedId())), 2)
end)

RegisterCommand('bothIndicators', function()
    if not IsPedInAnyVehicle(PlayerPedId()) then return end
    TriggerServerEvent('metaverse_carhud:syncIndicators', VehToNet(GetVehiclePedIsUsing(PlayerPedId())), 3)
end)

RegisterNetEvent("metaverse_carhud:syncIndicators", function(vehNetId, indicatorState)
...
RegisterKeyMapping('ceinture', 'Ceinture', 'keyboard', 'U')
...
RegisterCommand('ceinture', function()
    ...
    if IsPedInAnyVehicle(ply) then
        if antiSpam == false then
            if vehicleCategories ~= 13 or vehicleCategories ~= 8 then
