...
function PropertiesRefresh()
  ...
  if PropertiesList then
    ...
    for _, xPlayer in pairs(Players) do
      TriggerClientEvent("metaverse_property:syncProperties", xPlayer.source, Properties, xPlayer.get("lastProperty"))
    end
  else
    Properties = {}
...
CreateThread(function()
	...
	if PM.Enabled then
		...
		Wait(10)
		ESX.RefreshJobs()
    exports["metaverse_society"]:registerSociety('realestateagent', 'realestateagent', 'society_realestateagent', 'society_realestateagent', 'society_realestateagent', {type = 'private'})
	end
end)
...
AddEventHandler("esx:playerLoaded", function(playerId, xPlayer)
  ...
  MySQL.query("SELECT last_property FROM users WHERE identifier = ?", {xPlayer.identifier}, function(result)
    ...
    TriggerClientEvent("metaverse_property:syncProperties", playerId, Properties, lastProperty)
  end)
end)
...
ESX.RegisterCommand(_("create_name"), "user", function(xPlayer)
  if IsPlayerAdmin(xPlayer.source) or (PM.Enabled and xPlayer.job.name == PM.job) then
    xPlayer.triggerEvent("metaverse_property:CreateProperty")
  end
end, false,{help = TranslateCap("create_desc")})

ESX.RegisterCommand(_("admin_name"), Config.AllowedGroups, function(xPlayer)
  xPlayer.triggerEvent("metaverse_property:AdminMenu")
end, false,{help = TranslateCap("admin_desc")})

-- Buy Property
ESX.RegisterServerCallback("metaverse_property:buyProperty", function(source, cb, PropertyId)
  ...
  if xPlayer.getAccount("bank").money >= Price then
    ...
    TriggerClientEvent("metaverse_property:syncProperties", -1, Properties)
    if Config.OxInventory then
...
ESX.RegisterServerCallback("metaverse_property:attemptSellToPlayer", function(source, cb, PropertyId, PlayerId)
  local xPlayer = ESX.GetPlayerFromId(source)
  ...
  if xTarget and (xTarget.getAccount("bank").money >= Price) and (xPlayer.job.name == PM.job) then
    ...
    if PM.Enabled then
      ...
      TriggerEvent('metaverse_addonaccount:getSharedAccount', PM.society, function(account)
...
-- Buy Property
ESX.RegisterServerCallback("metaverse_property:buyFurniture", function(source, cb, PropertyId, PropName, PropIndex, PropCatagory, pos, heading)
  local xPlayer = ESX.GetPlayerFromId(source)
  ...
  if xPlayer.identifier == Owner or IsPlayerAdmin(source) or (Properties[PropertyId].Keys and Properties[PropertyId].Keys[xPlayer.identifier]) then
    ...
    if xPlayer.getAccount("bank").money >= Price then
      ...
      for i = 1, #Properties[PropertyId].plysinside do
        TriggerClientEvent("metaverse_property:placeFurniture", Properties[PropertyId].plysinside[i], PropertyId, furniture,
          #Properties[PropertyId].furniture)
      end
      TriggerClientEvent("metaverse_property:syncFurniture", -1, PropertyId, Properties[PropertyId].furniture)
    else
      cb(false)
...
ESX.RegisterServerCallback("metaverse_property:sellProperty", function(source, cb, PropertyId)
  local xPlayer = ESX.GetPlayerFromId(source)
  local Owner = Properties[PropertyId].Owner
...
ESX.RegisterServerCallback("metaverse_property:toggleLock", function(source, cb, PropertyId)
  local xPlayer = ESX.GetPlayerFromId(source)
  local Owner = Properties[PropertyId].Owner
...
ESX.RegisterServerCallback("metaverse_property:toggleGarage", function(source, cb, PropertyId)
  local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback("metaverse_property:toggleCCTV", function(source, cb, PropertyId)
  local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback("metaverse_property:SetGaragePos", function(source, cb, PropertyId, heading)
  local xPlayer = ESX.GetPlayerFromId(source)
  if IsPlayerAdmin(source, "ToggleGarage") then
...
ESX.RegisterServerCallback("metaverse_property:SetCCTVangle", function(source, cb, PropertyId, angles)
  local xPlayer = ESX.GetPlayerFromId(source)
  if IsPlayerAdmin(source, "ToggleCCTV") then
    local Property = Properties[PropertyId]
...
ESX.RegisterServerCallback("metaverse_property:CCTV", function(source, cb, PropertyId)
  local xPlayer = ESX.GetPlayerFromId(source)
  local Owner = Properties[PropertyId].Owner
  local Property = Properties[PropertyId]
  if xPlayer.identifier == Owner or IsPlayerAdmin(source) or (Properties[PropertyId].Keys and Properties[PropertyId].Keys[xPlayer.identifier]) then
...
ESX.RegisterServerCallback("metaverse_property:ExitCCTV", function(source, cb, PropertyId)
  local xPlayer = ESX.GetPlayerFromId(source)
  local Owner = Properties[PropertyId].Owner
  local Property = Properties[PropertyId]
  if xPlayer.identifier == Owner or IsPlayerAdmin(source) or (Properties[PropertyId].Keys and Properties[PropertyId].Keys[xPlayer.identifier]) then
...
ESX.RegisterServerCallback("metaverse_property:SetPropertyName", function(source, cb, PropertyId, name)
  local xPlayer = ESX.GetPlayerFromId(source)
  local Owner = Properties[PropertyId].Owner
  local Property = Properties[PropertyId]
...
ESX.RegisterServerCallback("metaverse_property:KnockOnDoor", function(source, cb, PropertyId, name)
  local xPlayer = ESX.GetPlayerFromId(source)
  local Property = Properties[PropertyId]
...
ESX.RegisterServerCallback("metaverse_property:RemoveCustomName", function(source, cb, PropertyId, name)
  local Property = Properties[PropertyId]
  local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback("metaverse_property:deleteProperty", function(source, cb, PropertyId)
  local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback("metaverse_property:ChangePrice", function(source, cb, PropertyId, NewPrice)
  local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback("metaverse_property:ChangeInterior", function(source, cb, PropertyId, Interior)
  local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback("metaverse_property:RemoveAllfurniture", function(source, cb, PropertyId)
  local xPlayer = ESX.GetPlayerFromId(source)
  local Owner = Properties[PropertyId].Owner
  if xPlayer.identifier == Owner or IsPlayerAdmin(source, "ResetFurniture") or
    ...
    for i = 1, #Properties[PropertyId].plysinside do
      for furniture = 1, #Properties[PropertyId].furniture do
        TriggerClientEvent("metaverse_property:removeFurniture", Properties[PropertyId].plysinside[i], PropertyId, furniture)
      end
    end
    Properties[PropertyId].furniture = {}
    TriggerClientEvent("metaverse_property:syncFurniture", -1, PropertyId, Properties[PropertyId].furniture)
...
ESX.RegisterServerCallback("metaverse_property:deleteFurniture", function(source, cb, PropertyId, furnitureIndex)
  local xPlayer = ESX.GetPlayerFromId(source)
  local Owner = Properties[PropertyId].Owner
  if xPlayer.identifier == Owner or IsPlayerAdmin(source) or (Properties[PropertyId].Keys and Properties[PropertyId].Keys[xPlayer.identifier]) then
    ...
    for i = 1, #Properties[PropertyId].plysinside do
      TriggerClientEvent("metaverse_property:removeFurniture", Properties[PropertyId].plysinside[i], PropertyId, furnitureIndex)
    end
  end
...
ESX.RegisterServerCallback("metaverse_property:editFurniture", function(source, cb, PropertyId, furnitureIndex, Pos, Heading)
  local xPlayer = ESX.GetPlayerFromId(source)
  local Owner = Properties[PropertyId].Owner
  if xPlayer.identifier == Owner or IsPlayerAdmin(source) or (Properties[PropertyId].Keys and Properties[PropertyId].Keys[xPlayer.identifier]) then
    if Properties[PropertyId].furniture[furnitureIndex] then
      ...
      TriggerClientEvent("metaverse_property:syncFurniture", -1, PropertyId, Properties[PropertyId].furniture)
      for i = 1, #Properties[PropertyId].plysinside do
        TriggerClientEvent("metaverse_property:editFurniture", Properties[PropertyId].plysinside[i], PropertyId, furnitureIndex, Pos, Heading)
      end
    end
  end
  cb(xPlayer.identifier == Owner or IsPlayerAdmin(source) or (Properties[PropertyId].Keys and Properties[PropertyId].Keys[xPlayer.identifier]))
...
ESX.RegisterServerCallback("metaverse_property:evictOwner", function(source, cb, PropertyId, Interior)
  local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback("metaverse_property:CanRaid", function(source, cb, PropertyId, Interior)
  local xPlayer = ESX.GetPlayerFromId(source)
  local Can = false
...
ESX.RegisterServerCallback("metaverse_property:ChangeEntrance", function(source, cb, PropertyId, Coords)
  local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback("metaverse_property:SetInventoryPosition", function(source, cb, PropertyId, Coords, Reset)
  if Config.OxInventory then
    local Property = Properties[PropertyId]
    local xPlayer = ESX.GetPlayerFromId(source)
...
-- Wardrobe
ESX.RegisterServerCallback("metaverse_property:SetWardrobePosition", function(source, cb, PropertyId, Coords, Reset)
  local Property = Properties[PropertyId]
  local xPlayer = ESX.GetPlayerFromId(source)
  if IsPlayerAdmin(source, "EditInteriorPositions") or (Property.Owner == xPlayer.identifier or Properties[PropertyId].Keys[xPlayer.identifier]) then
    local Interior = GetInteriorValues(Property.Interior)
...
ESX.RegisterServerCallback('metaverse_property:getPlayerDressing', function(source, cb)
  local xPlayer = ESX.GetPlayerFromId(source)

  TriggerEvent('metaverse_datastore:getDataStore', 'property', xPlayer.identifier, function(store)
...
ESX.RegisterServerCallback('metaverse_property:GetInsidePlayers', function(source, cb, property)
...
ESX.RegisterServerCallback('metaverse_property:GetNearbyPlayers', function(source, cb, property)
  local Property = Properties[property]
  local Players = {}
  local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('metaverse_property:GetPlayersWithKeys', function(source, cb, property)
  local Property = Properties[property]
  local Players = {}
  local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('metaverse_property:ShouldHaveKey', function(source, cb, property)
  local xPlayer = ESX.GetPlayerFromId(source)
  cb(Properties[property].Keys[xPlayer.identifier])
end)

ESX.RegisterServerCallback('metaverse_property:GetWebhook', function(source, cb, property)
  cb(Config.CCTV.PictureWebook)
end)

ESX.RegisterServerCallback('metaverse_property:RemoveLastProperty', function(source, cb, property)
  local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('metaverse_property:GiveKey', function(source, cb, property, player)
  local xPlayer = ESX.GetPlayerFromId(source)
  ...
  if Property.Owner == xPlayer.identifier then
    ...
    if not Properties[property].Keys[id] then
      ...
      xTarget.triggerEvent("metaverse_property:giveKeyAccess")
      cb(true)
    else
...
ESX.RegisterServerCallback('metaverse_property:StoreVehicle', function(source, cb, PropertyId, VehicleProperties)
  local xPlayer = ESX.GetPlayerFromId(source)
  local Property = Properties[PropertyId]
...
ESX.RegisterServerCallback('metaverse_property:AccessGarage', function(source, cb, PropertyId, VehicleProperties)
  local xPlayer = ESX.GetPlayerFromId(source)
  local Property = Properties[PropertyId]
...
ESX.RegisterServerCallback('metaverse_property:RemoveKey', function(source, cb, property, player)
  local xPlayer = ESX.GetPlayerFromId(source)
  ...
  if Property.Owner == xPlayer.identifier then
    if Property.Keys then
      if Properties[property].Keys[player] then
        ...
        xTarget.triggerEvent("metaverse_property:RemoveKeyAccess", property)
        cb(true)
      else
...
ESX.RegisterServerCallback('metaverse_property:CanOpenFurniture', function(source, cb, property)
  local xPlayer = ESX.GetPlayerFromId(source)
  local Property = Properties[property]
  cb(Property.Owner == xPlayer.identifier or (Property.Keys and Properties[property].Keys[xPlayer.identifier]))
end)

ESX.RegisterServerCallback('metaverse_property:getPlayerOutfit', function(source, cb, num)
  local xPlayer = ESX.GetPlayerFromId(source)
...
-- Player Management
if PM.Enabled then
  ESX.RegisterServerCallback('metaverse_property:PMenterOffice', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    local PlayerPed = GetPlayerPed(source)
  ...
  ESX.RegisterServerCallback('metaverse_property:PMexitOffice', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    local PlayerPed = GetPlayerPed(source)
...
RegisterNetEvent('metaverse_property:enter', function(PropertyId)
  local player = source
...
RegisterNetEvent('metaverse_property:leave', function(PropertyId)
  local player = source
  local Property = Properties[PropertyId]
  local xPlayer = ESX.GetPlayerFromId(player)
  MySQL.query("UPDATE `users` SET `last_property` = NULL WHERE `identifier` = ?", {xPlayer.identifier}) -- Remove Saved Data
...
RegisterNetEvent('metaverse_property:SetVehicleOut', function(PropertyId, VehIndex)
...
ESX.RegisterServerCallback('metaverse_property:CanCreateProperty', function(source, cb)
  local Re = false
  local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('metaverse_property:IsAdmin', function(source, cb)
  cb(IsPlayerAdmin(source, "ViewProperties"))
end)

ESX.RegisterServerCallback('metaverse_property:CanAccessRealEstateMenu', function(source, cb)
  local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('metaverse_property:server:createProperty', function(Property)
  local source = source
  local xPlayer = ESX.GetPlayerFromId(source)
  local Interior = GetInteriorValues(Property.interior)
  local garageData =
