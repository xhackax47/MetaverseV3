...
RegisterNetEvent("metaverse_property:syncProperties", function(properties, lastProperty)
  ...
  if lastProperty then
    if Properties[lastProperty.id] and (Properties[lastProperty.id].Owner == ESX.PlayerData.identifier or PlayerKeys[lastProperty.id]) then
      ...
      ESX.TriggerServerCallback('metaverse_property:RemoveLastProperty', function()
...
RegisterNetEvent("metaverse_property:giveKeyAccess", function(Property)
  ESX.TriggerServerCallback("metaverse_property:ShouldHaveKey", function(Should)
    if Should then
...
RegisterNetEvent("metaverse_property:RemoveKey", function(Property)
...
function GiveKeysMenu(Property)
  ESX.TriggerServerCallback("metaverse_property:GetInsidePlayers", function(Players)
    ...
    ESX.OpenContext("right", Elements, function(menu, element)
      ...
      elseif element.value == "user" then
        ESX.TriggerServerCallback("metaverse_property:GiveKey", function(KeyGiven)
          if KeyGiven then
...
function RemoveKeysMenu(Property)
  ESX.TriggerServerCallback("metaverse_property:GetPlayersWithKeys", function(Players)
    ...
    ESX.OpenContext("right", Elements, function(menu, element)
      if element.value == "go_back" then
        ManageKeys(Property)
      elseif element.value == "user" then
        ESX.TriggerServerCallback("metaverse_property:RemoveKey", function(KeyGiven)
          if KeyGiven then
...
function SetPropertyName(Property)
  ...
  ESX.OpenContext("right", Elements, function(menu, element)
    if element.name == "confirm" then
      ESX.TriggerServerCallback("metaverse_property:SetPropertyName", function(Set)
        if Set then
...
function OpenPropertyMenu(PropertyId)
  ...
  ESX.OpenContext("right", elements, function(menu, element)
    if element.value == "property_unlock" then
      ESX.TriggerServerCallback("metaverse_property:toggleLock", function(IsUnlocked)
        if IsUnlocked then
    ...
    if element.value == "property_raid" then
      ESX.TriggerServerCallback("metaverse_property:CanRaid", function(CanRaid)
        if CanRaid then
    ...
    if element.value == "property_inventory" then
      ...
      while SettingValue ~= "" do
        ...
        if IsControlJustPressed(0, 47) then
          SettingValue = ""
          ESX.HideUI()
          ESX.TriggerServerCallback("metaverse_property:SetInventoryPosition", function(Success)
            if Success then
    ...
    if element.value == "property_wardrobe" then
      ...
      while SettingValue ~= "" do
        ...
        if IsControlJustPressed(0, 47) then
          SettingValue = ""
          ESX.HideUI()
          ESX.TriggerServerCallback("metaverse_property:SetWardrobePosition", function(Success)
            if Success then
    ...
    if element.value == "property_buy" then
      ESX.TriggerServerCallback("metaverse_property:buyProperty", function(IsBought)
        if IsBought then
          local eles = PropertyMenuElements(PropertyId)
          exports["esx_context"]:Refresh(eles)
        else
    ...
    if element.value == "property_sell_re" then
      ...
      ESX.TriggerServerCallback("metaverse_property:GetNearbyPlayers", function(Players)
        if Players then
          for i = 1, #Players do
          ...
          ESX.OpenContext("right", Elements, function(menu, element)
            if element.value then
              ESX.TriggerServerCallback("metaverse_property:attemptSellToPlayer", function(IsBought)
                if IsBought then
                  local eles = PropertyMenuElements(PropertyId)
                  exports["esx_context"]:Refresh(eles)
                else
    ...
    if element.value == "property_knock" then
      ...
      ESX.TriggerServerCallback("metaverse_property:KnockOnDoor", function(HasKnocked)
    ...
    if element.value == "property_sell" then
      ESX.TriggerServerCallback("metaverse_property:sellProperty", function(IsSold)
        if IsSold then
          local eles = PropertyMenuElements(PropertyId)
          exports["esx_context"]:Refresh(eles)
        else
...
function AttemptHouseExit(PropertyId)
  ...
  Wait(1500)
  TriggerServerEvent("metaverse_property:leave", PropertyId)
  if Shell then
    DeleteObject(Shell)
    Shell = nil
  end
...
function AttemptHouseEntry(PropertyId)
  ...
  FreezeEntityPosition(ESX.PlayerData.ped, false)
  TriggerServerEvent("metaverse_property:enter", PropertyId)
  Wait(1500)
  DoScreenFadeIn(1800)
  InProperty = true
...
function StoreVehicle(PropertyId)
  ...
  if Vehicle then
    ...
    ESX.TriggerServerCallback("metaverse_property:StoreVehicle", function(result)
      if result then
...
function AccessGarage(PropertyId)
  ESX.TriggerServerCallback("metaverse_property:AccessGarage", function(Vehicles)
    if Vehicles then
          ...
          if ESX.Game.IsSpawnPointClear(vector3(Properties[PropertyId].garage.pos.x, Properties[PropertyId].garage.pos.y,
            ...
            ESX.Game.SpawnVehicle(element.Properties.model, Properties[PropertyId].garage.pos, Properties[PropertyId].garage.Heading,
              function(vehicle)
                ...
                TriggerServerEvent("metaverse_property:SetVehicleOut", PropertyId, element.index)
              end)
          end
        end
      end)
...
function OpenPMQuickMenu(Action)
  if Action == "Entrance" then
    DoScreenFadeOut(1500)
    Wait(1500)
    ESX.TriggerServerCallback("metaverse_property:PMenterOffice", function(HasEntered)
      if HasEntered then
  ...
  elseif Action == "Exit" then
    DoScreenFadeOut(1500)
    Wait(1500)
    ESX.TriggerServerCallback("metaverse_property:PMexitOffice", function(HasExited)
      if HasExited then
  ...
  elseif Action == "Properties" then
    TriggerEvent("metaverse_property:AdminMenu")
  ...
  elseif Action == "ActionsMenu" then
    ...
    ESX.OpenContext("right", elements, function(menu, element)
      if element.value == "CreateProperty" then
        TriggerEvent("metaverse_property:CreateProperty")
      end
...
ESX.RegisterInput(TranslateCap("realestate_command"), TranslateCap("realestate_command_desc"), "keyboard", "F5", function()
  ESX.TriggerServerCallback('metaverse_property:CanAccessRealEstateMenu', function(Access)
    if Access then
...
RegisterNetEvent("metaverse_property:CreateProperty", function()
  ESX.TriggerServerCallback('metaverse_property:CanCreateProperty', function(data)
    if data then
      local GetEntityCoords = GetEntityCoords
          ...
          if element.index then
            ...
            if element.index == "creation" then
              if HouseData.price and HouseData.name and HouseData.entrance and HouseData.interior then
                ...
                TriggerServerEvent("metaverse_property:server:createProperty", newProperty)
...
RegisterNetEvent("metaverse_property:AdminMenu", function()
  ESX.TriggerServerCallback('metaverse_property:IsAdmin', function(data)
    if data then
            ...
            ESX.OpenContext("right", GetData(), function(menu, element)
              if element.value then
                ...
                if element.value == "removeowner" then
                  ESX.TriggerServerCallback("metaverse_property:evictOwner", function(Evicted)
                    if Evicted then
                ...
                if element.value == "ToggleGarage" then
                  ESX.TriggerServerCallback("metaverse_property:toggleGarage", function(IsUnlocked, enabled)
                    if IsUnlocked then
                ...
                if element.value == "ToggleCCTV" then
                  ESX.TriggerServerCallback("metaverse_property:toggleCCTV", function(IsUnlocked, enabled)
                    if IsUnlocked then
                ...
                if element.value == "SetGaragePos" then
                  ...
                  while SettingValue ~= "" do
                    Wait(0)
                    if IsControlJustPressed(0, 38) then
                      ESX.TriggerServerCallback("metaverse_property:SetGaragePos", function(IsChanged)
                        if IsChanged then
                          ESX.HideUI()
                          SettingValue = ""
                  ...
                  while stage do
                    ...
                    if IsControlJustPressed(0, 38) then
                      ...
                      elseif stage == "maxleft" then
                        ...
                        ESX.TriggerServerCallback("metaverse_property:SetCCTVangle", function(IsChanged)
                          if IsChanged then
                            SettingValue = ""
                            stage = nil
                            ESX.HideUI()
                ...
                if element.value == "remove_custom_name" then
                  ESX.TriggerServerCallback("metaverse_property:RemoveCustomName", function(Cleared)
                    if Cleared then
                ...
                if element.value == "refurni" then
                  ESX.TriggerServerCallback("metaverse_property:RemoveAllfurniture", function(Removed)
                    if Removed then
                ...
                if element.value == "restorage" then
                  ESX.TriggerServerCallback("metaverse_property:SetInventoryPosition", function(Reset)
                    if Reset then
                ...
                if element.value == "rewardrobe" then
                  ESX.TriggerServerCallback("metaverse_property:SetWardrobePosition", function(Reset)
                    if Reset then
                ...
                if element.value == "price" then
                  ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'PropertyPrice', {title = "Property Price"}, function(data4, menu4)
                    if data4.value then
                      ESX.TriggerServerCallback("metaverse_property:ChangePrice", function(IsChanged)
                        if IsChanged then
                ...
                if element.value == "interior" then
                  ...
                  ESX.OpenContext("right", elements, function(menu, element)
                    if element.value then
                      ...
                      ESX.OpenContext("right", elements, function(menu, element)
                        if element.value then
                          ESX.TriggerServerCallback("metaverse_property:ChangeInterior", function(IsChanged)
                            if IsChanged then
                ...
                if element.value == "entrance" then
                  ESX.TriggerServerCallback("metaverse_property:ChangeEntrance", function(IsChanged)
                    if IsChanged then
            ...
            ESX.OpenContext("right", elements, function(menu, element)
              if element.value then
                ...
                if element.value == "delete" then
                  ESX.TriggerServerCallback("metaverse_property:deleteProperty", function(response)
                    if response then
