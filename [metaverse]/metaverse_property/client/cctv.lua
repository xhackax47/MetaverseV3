...
function CCTV(PropertyID)
  ...
  if Property.cctv.enabled then
    ESX.TriggerServerCallback("metaverse_property:CCTV", function(CanCCTV)
      if CanCCTV then
        InCCTV = true
        ...
        while IsCamActive(cctvcam) do
          ...
          if Config.CCTV.PictureWebook ~= "" and IsDisabledControlJustPressed(0, 201) then
            if CamTakePic then
              ...
              ESX.TriggerServerCallback("metaverse_property:GetWebhook", function(hook)
                if hook then
          ...
          if IsDisabledControlPressed(1, Config.CCTV.Controls.Exit) then
            ...
            ESX.TriggerServerCallback("metaverse_property:ExitCCTV", function(CanExit)
              if CanExit then
                InCCTV = false
                Wait(1000)
                ClearFocus()
