...
RegisterNetEvent('metaverse_lscustom:startModing', function(props, netId)
...
RegisterNetEvent('metaverse_lscustom:stopModing', function(plate)
	local src = tostring(source)
	if Customs[src] then
        ...
        for k, v in pairs(Customs[src]) do
            ...
            if DoesEntityExist(entity) then
                if playersCount > 0 then
                    TriggerClientEvent('metaverse_lscustom:restoreMods', -1, v.netId, v.props)
                else
                    DeleteEntity(entity)
                end
            end
...
RegisterNetEvent('metaverse_lscustom:buyMod', function(price)
	local source = source
  ...
  if not xPlayer then return print('^3[WARNING]^0 The player could\'nt be found.') end
	...
	if Config.IsMechanicJobOnly then
		...
		if price < societyAccount.money then
			TriggerClientEvent('metaverse_lscustom:installMod', source)
			TriggerClientEvent('esx:showNotification', source, TranslateCap('purchased'))
			societyAccount.removeMoney(price)
		else
			TriggerClientEvent('metaverse_lscustom:cancelInstallMod', source)
...
RegisterNetEvent('metaverse_lscustom:refreshOwnedVehicle', function(vehicleProps, netId)
	local src = tostring(source)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('metaverse_lscustom:getVehiclesPrices', function(source, cb)
