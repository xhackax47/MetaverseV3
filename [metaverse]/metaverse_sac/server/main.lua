...
ESX.RegisterUsableItem('bag', function(source)
    ...
    TriggerClientEvent('metaverse_sac:CheckBag', source, HasBag)
    if not HasBag then
        TriggerEvent('metaverse_sac:InsertBag', source)
    else
        TriggerClientEvent('esx:showNotification', source, '~r~You already have a bag on you.')
    end
end)

ESX.RegisterServerCallback('metaverse_sac:getPlayerInventory', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('metaverse_sac:getBag', function(source, cb)
    local src = source
...
ESX.RegisterServerCallback('metaverse_sac:getAllBags', function(source, cb)
    local src = source
...
ESX.RegisterServerCallback('metaverse_sac:getBagInventory', function(source, cb, BagId)
    local src = source
    local identifier = ESX.GetPlayerFromId(src).identifier
...
RegisterServerEvent('metaverse_sac:InsertBag')
AddEventHandler('metaverse_sac:InsertBag', function(source)
    local src = source
    local identifier = ESX.GetPlayerFromId(src).identifier
    local xPlayer = ESX.GetPlayerFromId(src)
    local xPlayers = ESX.GetPlayers()

    TriggerClientEvent('metaverse_sac:GiveBag', src)
    for i=1, #xPlayers, 1 do
        TriggerClientEvent('metaverse_sac:ReSync', xPlayers[i], id)
     end
...
RegisterServerEvent('metaverse_sac:TakeItem')
AddEventHandler('metaverse_sac:TakeItem', function(id, item, count, type)
    local src = source
    local identifier = ESX.GetPlayerFromId(src).identifier
    local xPlayer = ESX.GetPlayerFromId(src)
...
RegisterServerEvent('metaverse_sac:PutItem')
AddEventHandler('metaverse_sac:PutItem', function(id, item, label, count, type)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    local identifier = ESX.GetPlayerFromId(src).identifier
	local update
...
RegisterServerEvent('metaverse_sac:PickUpBag')
AddEventHandler('metaverse_sac:PickUpBag', function(id)
    local src = source
    local identifier = ESX.GetPlayerFromId(src).identifier
    ...
    MySQL.Async.fetchAll('UPDATE owned_bags SET identifier = @identifier, x = @x, y = @y, z = @z WHERE id = @id', {['@identifier'] = identifier, ['@id'] = id, ['@x'] = nil, ['@y'] = nil, ['@z'] = nil})
     ...
     for i=1, #xPlayers, 1 do
        TriggerClientEvent('metaverse_sac:SetOntoPlayer', src, id)
	TriggerClientEvent('esx:showNotification', src, 'Appuyer sur ~b~[F2]~b~ pour accéder au sac.')
        TriggerClientEvent('metaverse_sac:ReSync', xPlayers[i], id)
     end
end)

RegisterServerEvent('metaverse_sac:DropBag')
AddEventHandler('metaverse_sac:DropBag', function(id, x, y, z)
    local src = source
    local identifier = ESX.GetPlayerFromId(src).identifier
