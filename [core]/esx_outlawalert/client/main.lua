...
RegisterNetEvent('esx_outlawalert:selldrugInProgress')
AddEventHandler('esx_outlawalert:selldrugInProgress', function(targetCoords)
	if isPlayerWhitelisted and Config.DrugSaleAlert then
		local alpha = 250
		local drugBlip = AddBlipForRadius(targetCoords.x, targetCoords.y, targetCoords.z, Config.BlipDrugSaleRadius)
		...
		while alpha ~= 0 do
			Citizen.Wait(Config.BlipDrugSaleTime * 4)
			alpha = alpha - 1
			SetBlipAlpha(drugBlip, alpha)
...
Citizen.CreateThread(function()
	while true do
		...
		--[[
		elseif exports['ti_drugSales']:sellDrugs(playerId) and Config.DrugSaleAlert then
			...
			if (isPlayerWhitelisted and Config.ShowCopsMisbehave) or not isPlayerWhitelisted then
				...
				TriggerServerEvent('esx_outlawalert:selldrugInProgress', {
					x = ESX.Math.Round(playerCoords.x, 1),
					y = ESX.Math.Round(playerCoords.y, 1),
					z = ESX.Math.Round(playerCoords.z, 1)
				}, streetName, playerGender)
