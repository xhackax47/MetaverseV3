...
RegisterServerEvent('esx_outlawalert:selldrugInProgress')
AddEventHandler('esx_outlawalert:selldrugInProgress', function(targetCoords, streetName, playerGender)
	if playerGender == 0 then
		playerGender = _U('male')
	else
		playerGender = _U('female')
	...
	TriggerClientEvent('esx_outlawalert:outlawNotify', -1, _U('selldrug', playerGender, streetName))
