...
RegisterNUICallback("volumeUp", function()
	if RadioVolume <= 95 then
		...
		exports["mumble-voip"]:SetMumbleProperty("volume", RadioVolume)
	else
...
RegisterNUICallback("increaseradiochannel", function(data, cb)
    ...
    exports["mumble-voip"]:SetRadioChannel(RadioChannel)
...
CreateThread(function()
    while Config.Item.Require do
        ...
        if ESX.IsPlayerLoaded() and onRadio then
            ESX.TriggerServerCallback("esx_radio:server:GetItem", function(hasItem)
                if not hasItem then
                    if RadioChannel ~= 0 then
                        leaveradio()
                    end
...
for i=1, Config.MaxFrequency do
    RegisterNetEvent('esx_radio:client:JoinRadioChannel'.. i, function(channel)
        exports["mumble-voip"]:SetRadioChannel(i)
...
if Config.KeyMappings.Enabled then
    RegisterKeyMapping("radio", 'Utiliser la radio', 'keyboard', Config.KeyMappings.Key)
end
