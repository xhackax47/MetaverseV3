$(function() {
    ...
    document.onkeyup = function (data) {
        if (data.which == 27) { // Escape key
            $.post('https://esx_radio/escape', JSON.stringify({}));
            ESXRadio.SlideDown()
        } else if (data.which == 13) { // Enter key
            $.post('https://esx_radio/joinRadio', JSON.stringify({
                channel: $("#channel").val()
            }));
        }
    };
...
$(document).on('click', '#disconnect', function(e){
    ...
    $.post('https://esx_radio/leaveRadio');
});

$(document).on('click', '#volumeUp', function(e){
    e.preventDefault();

    $.post('https://esx_radio/volumeUp', JSON.stringify({
        channel: $("#channel").val()
    }));
});
...
$(document).on('click', '#volumeDown', function(e){
    ...
    $.post('https://esx_radio/volumeDown', JSON.stringify({
        channel: $("#channel").val()
    }));
});
...
$(document).on('click', '#decreaseradiochannel', function(e){
    ...
    $.post('https://esx_radio/decreaseradiochannel', JSON.stringify({
        channel: $("#channel").val()
    }));
});
...
$(document).on('click', '#increaseradiochannel', function(e){
    ...
    $.post('https://esx_radio/increaseradiochannel', JSON.stringify({
        channel: $("#channel").val()
    }));
});
...
$(document).on('click', '#poweredOff', function(e){
    ...
    $.post('https://esx_radio/poweredOff', JSON.stringify({
        channel: $("#channel").val()
    }));
});
