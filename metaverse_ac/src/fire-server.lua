...
AddEventHandler("FIREAC:BanFromClient", function (ACTION ,REASON, DETAILS)
    ...
    if REASON ~= nil and ACTION ~= nil then
        ...
        FIREAC_ERROR(SERVER_NAME, "FIREAC:BanFromClient : REASON or ACTION (Non trouvé)")
    end
end)
...
AddEventHandler("FIREAC:BanForInject", function (REASON, DETAILS, RESOURCE)
    ...
    if REASON ~= nil and RESOURCE ~= nil then
        ...
        FIREAC_ERROR(SERVER_NAME, "FIREAC:BanForInject : REASON or RESOURCE (Non trouvé)")
    end
end)
...
AddEventHandler("FIREAC:ScreenShotFromClient", function (URL, REASON, DETAILS)
    ...
    if tonumber(SRC) ~= nil and GetPlayerName(SRC) ~= nil then
        ...
        local STEAM   = "Non trouvé"
        local DISCORD = "Non trouvé"
        local FIVEML  = "Non trouvé"
        local LIVE    = "Non trouvé"
        local XBL     = "Non trouvé"
        local ISP     = "Non trouvé"
        local CITY    = "Non trouvé"
        local COUNTRY = "Non trouvé"
        local PROXY   = "Non trouvé"
        local HOSTING = "Non trouvé"
                        ...
                        if URL ~= nil then
                           PerformHttpRequest(FIREAC.ScreenShot.Log, function(ERROR, DATA, RESULT)
                                ...
                                embeds = {
                                    {
                                        author = {
                                            name = ""..Emoji.Fire.."| METAVERSE AC™ | "..Emoji.Fire.."",
                                        ...
                                        footer = {
                                            text = "METAVERSE AC "..Emoji.Fire.." | "..os.date("%Y/%m/%d | %X").."",
                                            icon_url = "https://cdn.discordapp.com/attachments/837386511920922694/838343457700839434/3928fa3aa4971eeb3d88482c62540344.png",
                                        },
        ...
        FIREAC_ERROR(SERVER_NAME, "FIREAC:ScreenShotFromClient (SRC Non trouvé)")
    end
end)
...
AddEventHandler("playerDropped", function(REASON)
    ...
    if GetPlayerName(SRC) and REASON ~= nil then
        ...
        FIREAC_ERROR(SERVER_NAME, "playerDropped : REASON or SRC (Non trouvé)")
    end
end)
...
AddEventHandler("giveWeaponEvent", function(SRC, DATA)
    if FIREAC.AntiAddWeapon then
        if tonumber(SRC) ~= nil and GetPlayerName(SRC) ~= nil then
            ...
            FIREAC_ERROR(SERVER_NAME, "giveWeaponEvent : SRC (Non trouvé)")
        end
    end
end)
...
AddEventHandler("playerConnecting", function (name, setKickReason, deferrals)
    ...
    local PROXY   = "Non trouvé"
    local HOSTING = "Non trouvé"
    local LON     = "Non trouvé"
    local LAT     = "Non trouvé"
    local HWID    = GetPlayerToken(SRC, 0)
    IP = (string.gsub(string.gsub(string.gsub(IP,  "-", ""), ",", ""), " ", ""):lower())
    local g, f = IP:find(string.lower("192.168"))
    if g or f then
    ...
    if BANFILE ~= nil then
        ...
        if TABLE ~= nil and type(TABLE) == "table" then
            if tonumber(SRC) ~= nil then
                ...
                local XBL     = "Non trouvé"
                local IP = GetPlayerEndpoint(SRC)
                local BANID   =  "Non trouvé"
                local REASON  = "Non trouvé"
                local BANNED  = false
                IP = (string.gsub(string.gsub(string.gsub(IP,  "-", ""), ",", ""), " ", ""):lower())
                local g, f = IP:find(string.lower("192.168"))
                if g or f then
        ...
        PerformHttpRequest("http://ip-api.com/json/"..IP.."?fields=66846719", function(ERROR, DATA, RESULT)
            if DATA ~= nil then
                ...
                if TABLE ~= nil then
                    ...
                    FIREAC_ERROR(SERVER_NAME, "playerConnecting (TABLE Non trouvé)")
                end
            else
                FIREAC_ERROR(SERVER_NAME, "playerConnecting (DATA Non trouvé)")
            end
        end)
    else
        FIREAC_SENDLOG(SRC, FIREAC.Log.Connect, "CONNECT")
...
function StartAntiCheat()
        ...
        fire_emoji
        then
    print("^"..COLORS.."[METAVERSE_AC]^0: ^2 Chargé !^0")
        PerformHttpRequest("http://localhost:"..FIREAC.Port.."/info.json", function(ERROR, DATA, RESULT)
            if DATA ~= nil then
                ...
                print("^"..COLORS.."[METAVERSE AC]^0: ^3Server Build : "..ART7.."")
                ...
                embeds = {
                    {
                        ...
                        title = "METAVERSE AC "..FIREAC.Version.."",
        ...
        print("^"..COLORS.."[METAVERSE AC]^0: ^1 Un fichier de votre METAVERSE AC n'a pas été trouvé ! Veuillez le remplacer ou le réparer^0")
        Wait(1000)
        print("^"..COLORS.."[METAVERSE AC]^0: ^1 Un fichier de votre METAVERSE AC n'a pas été trouvé ! Veuillez le remplacer ou le réparer^0")
        Wait(1000)
        print("^"..COLORS.."[METAVERSE AC]^0: ^1 Un fichier de votre METAVERSE AC n'a pas été trouvé ! Veuillez le remplacer ou le réparer^0")
...
function FIREAC_WHITELIST(SRC)
     if tonumber(SRC) ~= nil then
        ...
        FIREAC_ERROR(SERVER_NAME, "function FIREAC_WHITELIST (SRC Non trouvé)")
    end
end
...
function FIREAC_ERROR(SERVER_NAME, ERROR)
    if SERVER_NAME ~= nil then
        if ERROR ~= nil then
            ...
            FIREAC_ERROR(SERVER_NAME, "function FIREAC_ERROR (ERROR Non trouvé)")
        end
    else
        FIREAC_ERROR(SERVER_NAME, "function FIREAC_ERROR (SERVER_NAME Non trouvé)")
    end
end
...
function FIREAC_ACTION(SRC, ACTION, REASON, DETAILS)
    if REASON ~= nil and DETAILS ~= nil then
        ...
        FIREAC_ERROR(SERVER_NAME, "function FIREAC_ACTION (REASON and DETAILS Non trouvé)")
    end
end
    ...
    if TYPE ~= nil then
        if NAME ~= nil then
            if REASON ~= nil then
                ...
                FIREAC_ERROR(SERVER_NAME, "function FIREAC_MEESAGE (REASON Non trouvé)")
            end
            else
            FIREAC_ERROR(SERVER_NAME, "function FIREAC_MEESAGE (NAME Non trouvé)")
        end
    else
        FIREAC_ERROR(SERVER_NAME, "function FIREAC_MEESAGE (TYPE Non trouvé)")
    end
end
        ...
        PerformHttpRequest("http://ip-api.com/json/"..IP.."?fields=66846719", function(ERROR, DATA, RESULT)
            if DATA ~= nil then
                ...
                if TABLE ~= nil then
                    ...
                    FIREAC_ERROR(SERVER_NAME, " FIREAC_SENDLOG (in IP API DATA Non trouvé )")
                end
            end
        end)
    else
